...
contract('Pool Manager: Sum-Product rounding errors', async accounts => {
  ...
  it.skip("Rounding errors: 100 deposits of 100ANTUSD into SP, then 200 liquidations of 49ANTUSD", async () => {
    ...
    const SP_TotalDeposits = await stabilityPool.getTotalANTUSDDeposits()
    const SP_ETH = await stabilityPool.getETH()
    const compoundedDeposit = await stabilityPool.getCompoundedANTUSDDeposit(depositors[0])
