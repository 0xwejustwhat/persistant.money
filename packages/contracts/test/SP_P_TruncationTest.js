...
contract('StabilityPool Scale Factor issue tests', async accounts => {
  ...
  const getANTUSDAmountForDesiredDebt = async (desiredDebt) => (await getOpenTroveANTUSDAmount(dec(desiredDebt, 18))).add(th.toBN(1))
  ...
  it("1. Liquidation succeeds after P reduced to 1", async () => {
    ...
    for (account of [A, B, C]) {
      await borrowerOperations.openTrove(th._100pct, await getANTUSDAmountForDesiredDebt(2000), account, account, dec(15, 'ether'), {from: account })
    ...
    const deposit_1 = deposit_0.sub(await stabilityPool.getTotalANTUSDDeposits())
    ...
    const deposit_2 = deposit_0.sub(await stabilityPool.getTotalANTUSDDeposits())
  ...
  it("3. Liquidation succeeds when P == 1 and liquidation has newProductFactor == 1e9", async () => {
    ...
    const deposit_2 = deposit_0.sub(await stabilityPool.getTotalANTUSDDeposits()).add(th.toBN(dec(2,12)))
    await stabilityPool.provideToSP(deposit_2, ZERO_ADDRESS, {from: A})
  ...
  it("4. Liquidation succeeds when P == 1 and liquidation has newProductFactor > 1e9", async () => {
    ...
    const deposit_2 = deposit_0.mul(th.toBN(2)).sub(await stabilityPool.getTotalANTUSDDeposits())
    await stabilityPool.provideToSP(deposit_2, ZERO_ADDRESS, {from: A})
  ...
  it("5. Depositor have correct depleted stake after deposit at P == 1 and scale changing liq (with newProductFactor == 1e9)", async () => {
    ...
    const D_depletedDeposit = await stabilityPool.getCompoundedANTUSDDeposit(D)
