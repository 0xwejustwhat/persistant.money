...
const ANTUSDTokenTester = artifacts.require("./ANTUSDTokenTester")
...
/* NOTE: Some of the borrowing tests do not test for specific ANTUSD fee values. They only test that the
 * fees are non-zero when they should occur, and that they decay over time.
 *
 * Specific ANTUSD fee values will depend on the final fee schedule used, and the final choice for
...
contract('BorrowerOperations', async accounts => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    it("addColl(): Increases the activePool ETH and raw ether balance by correct amount", async () => {
      ...
      const activePool_RawEther_Before = toBN(await contracts.stETH.balanceOf(activePool.address))
      ...
      const activePool_RawEther_After = toBN(await contracts.stETH.balanceOf(activePool.address))
    ...
    it("addColl(), active Trove: applies pending rewards and updates user's L_ETH, L_ANTUSDDebt snapshots", async () => {
      // --- SETUP ---

      const { collateral: aliceCollBefore, totalDebt: aliceDebtBefore } = await openTrove({ extraANTUSDAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      const { collateral: bobCollBefore, totalDebt: bobDebtBefore } = await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extraANTUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })

      // --- TEST ---

      // price drops to 1ETH:100ANTUSD, reducing Carol's ICR below MCR
      ...
      const L_ANTUSDDebt = await troveManager.L_ANTUSDDebt()
      ...
      const alice_ANTUSDDebtRewardSnapshot_Before = alice_rewardSnapshot_Before[1]
      ...
      const bob_ANTUSDDebtRewardSnapshot_Before = bob_rewardSnapshot_Before[1]

      assert.equal(alice_ETHrewardSnapshot_Before, 0)
      assert.equal(alice_ANTUSDDebtRewardSnapshot_Before, 0)
      assert.equal(bob_ETHrewardSnapshot_Before, 0)
      assert.equal(bob_ANTUSDDebtRewardSnapshot_Before, 0)
      ...
      const alicePendingANTUSDDebtReward = await troveManager.getPendingANTUSDDebtReward(alice)
      const bobPendingANTUSDDebtReward = await troveManager.getPendingANTUSDDebtReward(bob)
      for (reward of [alicePendingETHReward, bobPendingETHReward, alicePendingANTUSDDebtReward, bobPendingANTUSDDebtReward]) {
      ...
      assert.isTrue(aliceNewDebt.eq(aliceDebtBefore.add(alicePendingANTUSDDebtReward)))
      assert.isTrue(bobNewColl.eq(bobCollBefore.add(bobPendingETHReward).add(bobTopUp)))
      assert.isTrue(bobNewDebt.eq(bobDebtBefore.add(bobPendingANTUSDDebtReward)))

      /* Check that both Alice and Bob's snapshots of the rewards-per-unit-staked metrics should be updated
       to the latest values of L_ETH and L_ANTUSDDebt */
      ...
      const alice_ANTUSDDebtRewardSnapshot_After = alice_rewardSnapshot_After[1]
      ...
      const bob_ANTUSDDebtRewardSnapshot_After = bob_rewardSnapshot_After[1]

      assert.isAtMost(th.getDifference(alice_ETHrewardSnapshot_After, L_ETH), 100)
      assert.isAtMost(th.getDifference(alice_ANTUSDDebtRewardSnapshot_After, L_ANTUSDDebt), 100)
      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot_After, L_ETH), 100)
      assert.isAtMost(th.getDifference(bob_ANTUSDDebtRewardSnapshot_After, L_ANTUSDDebt), 100)
    })
    ...
    //   // --- SETUP ---
    //   // A,B,C add 15/5/5 ETH, withdraw 100/100/900 ANTUSD
    ...
    //   // close Carol's Trove, liquidating her 5 ether and 900ANTUSD.
    ...
    it("withdrawColl(): reverts when calling address does not have active trove", async () => {
      ...
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
    ...
    it("withdrawColl(): reverts if system is in Recovery Mode", async () => {
      ...
      // price drops to 1ETH:150ANTUSD, reducing TCR below 150%
    ...
    it("withdrawColl(): sends the correct amount of ETH to the user", async () => {
      ...
      const alice_ETHBalance_Before = toBN(web3.utils.toBN(await contracts.stETH.balanceOf(alice)))
      await borrowerOperations.withdrawColl(dec(1, 'ether'), alice, alice, { from: alice, gasPrice: 0 })

      const alice_ETHBalance_After = toBN(web3.utils.toBN(await contracts.stETH.balanceOf(alice)))
      const balanceDiff = alice_ETHBalance_After.sub(alice_ETHBalance_Before)

      assert.isTrue(balanceDiff.eq(toBN(dec(1, 'ether'))))
    })

    it("withdrawColl(): applies pending rewards and updates user's L_ETH, L_ANTUSDDebt snapshots", async () => {
      // --- SETUP ---
      ...
      // close Carol's Trove, liquidating her 1 ether and 180ANTUSD.
      await troveManager.liquidate(carol, { from: owner });
      ...
      const pendingDebtReward_A = await troveManager.getPendingANTUSDDebtReward(alice)
      const pendingCollReward_B = await troveManager.getPendingETHReward(bob)
      const pendingDebtReward_B = await troveManager.getPendingANTUSDDebtReward(bob)
    ...
    it("withdrawANTUSD(): reverts when withdrawal would leave trove with ICR < MCR", async () => {
      // alice creates a Trove and adds first collateral
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
      ...
      const ANTUSDwithdrawal = 1  // withdraw 1 wei ANTUSD

     await assertRevert(borrowerOperations.withdrawANTUSD(th._100pct, ANTUSDwithdrawal, alice, alice, { from: alice }), 
      "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })

    it("withdrawANTUSD(): decays a non-zero base rate", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      await openTrove({ extraANTUSDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraANTUSDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      ...
      // D withdraws ANTUSD
      await borrowerOperations.withdrawANTUSD(th._100pct, dec(1, 18), A, A, { from: D })
      ...
      // E withdraws ANTUSD
      await borrowerOperations.withdrawANTUSD(th._100pct, dec(1, 18), A, A, { from: E })

      const baseRate_3 = await troveManager.baseRate()
      assert.isTrue(baseRate_3.lt(baseRate_2))
    })

    it("withdrawANTUSD(): reverts if max fee > 100%", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraANTUSDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraANTUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraANTUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })

      await assertRevert(borrowerOperations.withdrawANTUSD(dec(2, 18), dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdrawANTUSD('1000000000000000001', dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
    })

    it("withdrawANTUSD(): reverts if max fee < 0.5% in Normal mode", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraANTUSDAmount: toBN(dec(20, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraANTUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraANTUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })

      await assertRevert(borrowerOperations.withdrawANTUSD(0, dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdrawANTUSD(1, dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
      await assertRevert(borrowerOperations.withdrawANTUSD('4999999999999999', dec(1, 18), A, A, { from: A }), "Max fee percentage must be between 0.5% and 100%")
    })

    it("withdrawANTUSD(): reverts if fee exceeds max fee percentage", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraANTUSDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraANTUSDAmount: toBN(dec(70, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      ...
      // ANTUSDFee:                  15000000558793542
      // absolute _fee:            15000000558793542
      // actual feePercentage:      5000000186264514
      // user's _maxFeePercentage: 49999999999999999

      const lessThan5pct = '49999999999999999'
      await assertRevert(borrowerOperations.withdrawANTUSD(lessThan5pct, dec(3, 18), A, A, { from: A }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate() // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 1%
      await assertRevert(borrowerOperations.withdrawANTUSD(dec(1, 16), dec(1, 18), A, A, { from: B }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate()  // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 3.754%
      await assertRevert(borrowerOperations.withdrawANTUSD(dec(3754, 13), dec(1, 18), A, A, { from: C }), "Fee exceeded provided maximum")

      baseRate = await troveManager.baseRate()  // expect 5% base rate
      assert.equal(baseRate, dec(5, 16))
      // Attempt with maxFee 0.5%%
      await assertRevert(borrowerOperations.withdrawANTUSD(dec(5, 15), dec(1, 18), A, A, { from: D }), "Fee exceeded provided maximum")
    })

    it("withdrawANTUSD(): succeeds when fee is less than max fee percentage", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraANTUSDAmount: toBN(dec(60, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraANTUSDAmount: toBN(dec(70, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
      await openTrove({ extraANTUSDAmount: toBN(dec(80, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      ...
      const tx1 = await borrowerOperations.withdrawANTUSD(moreThan5pct, dec(1, 18), A, A, { from: A })
      ...
      // Attempt with maxFee = 5%
      const tx2 = await borrowerOperations.withdrawANTUSD(dec(5, 16), dec(1, 18), A, A, { from: B })
      ...
      const tx3 = await borrowerOperations.withdrawANTUSD(dec(1, 17), dec(1, 18), A, A, { from: C })
      ...
      const tx4 = await borrowerOperations.withdrawANTUSD(dec(37659, 13), dec(1, 18), A, A, { from: D })
      assert.isTrue(tx4.receipt.status)

      // Attempt with maxFee 100%
      const tx5 = await borrowerOperations.withdrawANTUSD(dec(1, 18), dec(1, 18), A, A, { from: E })
      assert.isTrue(tx5.receipt.status)
    })

    it("withdrawANTUSD(): doesn't change base rate if it is already zero", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      await openTrove({ extraANTUSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      ...
      // D withdraws ANTUSD
      await borrowerOperations.withdrawANTUSD(th._100pct, dec(37, 18), A, A, { from: D })
      ...
      // E opens trove 
      await borrowerOperations.withdrawANTUSD(th._100pct, dec(12, 18), A, A, { from: E })

      const baseRate_3 = await troveManager.baseRate()
      assert.equal(baseRate_3, '0')
    })

    it("withdrawANTUSD(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      ...
      await borrowerOperations.withdrawANTUSD(th._100pct, dec(1, 18), C, C, { from: C })
    ...
    it("withdrawANTUSD(): borrower can't grief the baseRate and stop it decaying by issuing debt at higher frequency than the decay granularity", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraANTUSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraANTUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraANTUSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("withdrawANTUSD(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        ...
        const withdrawalTx = await borrowerOperations.withdrawANTUSD(th._100pct, toBN(dec(37, 18)), D, D, { from: D })

        const emittedFee = toBN(th.getANTUSDFeeFromANTUSDBorrowingEvent(withdrawalTx))
    ...
    it("withdrawANTUSD(): Borrowing at zero base rate changes ANTUSD fees-per-unit-staked", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraANTUSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraANTUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraANTUSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("withdrawANTUSD(): Borrowing at zero base rate sends debt request to user", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraANTUSDAmount: toBN(dec(30, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraANTUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraANTUSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("withdrawANTUSD(): reverts when calling address does not have active trove", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })

      // Bob successfully withdraws ANTUSD
      const txBob = await borrowerOperations.withdrawANTUSD(th._100pct, dec(100, 18), bob, bob, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Carol with no active trove attempts to withdraw ANTUSD
      try {
        const txCarol = await borrowerOperations.withdrawANTUSD(th._100pct, dec(100, 18), carol, carol, { from: carol })
        assert.isFalse(txCarol.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("withdrawANTUSD(): reverts when requested withdrawal amount is zero ANTUSD", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })

      // Bob successfully withdraws 1e-18 ANTUSD
      const txBob = await borrowerOperations.withdrawANTUSD(th._100pct, 1, bob, bob, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Alice attempts to withdraw 0 ANTUSD
      try {
        const txAlice = await borrowerOperations.withdrawANTUSD(th._100pct, 0, alice, alice, { from: alice })
    ...
    it("withdrawANTUSD(): reverts when system is in Recovery Mode", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
      ...
      // Withdrawal possible when recoveryMode == false
      const txAlice = await borrowerOperations.withdrawANTUSD(th._100pct, dec(100, 18), alice, alice, { from: alice })
      assert.isTrue(txAlice.receipt.status)
      ...
      //Check ANTUSD withdrawal impossible when recoveryMode == true
      try {
        const txBob = await borrowerOperations.withdrawANTUSD(th._100pct, 1, bob, bob, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
    ...
    it("withdrawANTUSD(): reverts when withdrawal would bring the trove's ICR < MCR", async () => {
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(11, 17)), extraParams: { from: bob } })

      // Bob tries to withdraw ANTUSD that would bring his ICR < MCR
      try {
        const txBob = await borrowerOperations.withdrawANTUSD(th._100pct, 1, bob, bob, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
    ...
    it("withdrawANTUSD(): reverts when a withdrawal would cause the TCR of the system to fall below the CCR", async () => {
      await priceFeed.setPrice(dec(100, 18))
      const price = await priceFeed.getPrice()
      ...
      // Bob attempts to withdraw 1 ANTUSD.
      // System TCR would be: ((3+3) * 100 ) / (200+201) = 600/401 = 149.62%, i.e. below CCR of 150%.
      try {
        const txBob = await borrowerOperations.withdrawANTUSD(th._100pct, dec(1, 18), bob, bob, { from: bob })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("withdrawANTUSD(): reverts if system is in Recovery Mode", async () => {
      // --- SETUP ---
      await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(15, 17)), extraParams: { from: bob } })
      ...
      try {
        const txData = await borrowerOperations.withdrawANTUSD(th._100pct, '200', alice, alice, { from: alice })
        assert.isFalse(txData.receipt.status)
      } catch (err) {
        assert.include(err.message, 'revert')
      }
    ...
    it("withdrawANTUSD(): increases the Trove's ANTUSD debt by the correct amount", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      await borrowerOperations.withdrawANTUSD(th._100pct, await getNetBorrowingAmount(100), alice, alice, { from: alice })
    ...
    it("withdrawANTUSD(): increases ANTUSD debt in ActivePool by correct amount", async () => {
      ...
      // check before
      const activePool_ANTUSD_Before = await activePool.getANTUSDDebt()
      assert.isTrue(activePool_ANTUSD_Before.eq(aliceDebtBefore))

      await borrowerOperations.withdrawANTUSD(th._100pct, await getNetBorrowingAmount(dec(10000, 18)), alice, alice, { from: alice })

      // check after
      const activePool_ANTUSD_After = await activePool.getANTUSDDebt()
      th.assertIsApproximatelyEqual(activePool_ANTUSD_After, activePool_ANTUSD_Before.add(toBN(dec(10000, 18))))
    })

    it("withdrawANTUSD(): increases user ANTUSDToken balance by correct amount", async () => {
    ...
    // --- repayANTUSD() ---
    it("repayANTUSD(): reverts when repayment would leave trove with ICR < MCR", async () => {
      // alice creates a Trove and adds first collateral
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
      ...
      const ANTUSDRepayment = 1  // 1 wei repayment

     await assertRevert(borrowerOperations.repayANTUSD(ANTUSDRepayment, alice, alice, { from: alice }), 
      "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })

    it("repayANTUSD(): Succeeds when it would leave trove with net debt >= minimum net debt", async () => {
      // Make the ANTUSD request 2 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
      await borrowerOperations.openTrove(th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN('2'))), A, A, dec(100, 30), { from: A})

      const repayTxA = await borrowerOperations.repayANTUSD(1, A, A, { from: A })
      assert.isTrue(repayTxA.receipt.status)

      await borrowerOperations.openTrove(th._100pct, dec(20, 25), B, B, dec(100, 30), { from: B })

      const repayTxB = await borrowerOperations.repayANTUSD(dec(19, 25), B, B, { from: B })
      assert.isTrue(repayTxB.receipt.status)
    })

    it("repayANTUSD(): reverts when it would leave trove with net debt < minimum net debt", async () => {
      // Make the ANTUSD request 2 wei above min net debt to correct for floor division, and make net debt = min net debt + 1 wei
      await borrowerOperations.openTrove(th._100pct, await getNetBorrowingAmount(MIN_NET_DEBT.add(toBN('2'))), A, A, dec(100, 30), { from: A })

      const repayTxAPromise = borrowerOperations.repayANTUSD(2, A, A, { from: A })
    ...
    it("adjustTrove(): Reverts if repaid amount is greater than current debt", async () => {
      const { totalDebt } = await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      ANTUSD_GAS_COMPENSATION = await borrowerOperations.ANTUSD_GAS_COMPENSATION()
    ...
    it("repayANTUSD(): reverts when calling address does not have active trove", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      // Bob successfully repays some ANTUSD
      const txBob = await borrowerOperations.repayANTUSD(dec(10, 18), bob, bob, { from: bob })
      assert.isTrue(txBob.receipt.status)

      // Carol with no active trove attempts to repayANTUSD
      try {
        const txCarol = await borrowerOperations.repayANTUSD(dec(10, 18), carol, carol, { from: carol })
        assert.isFalse(txCarol.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("repayANTUSD(): reverts when attempted repayment is > the debt of the trove", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      ...
      try {
        const txAlice = await borrowerOperations.repayANTUSD(aliceDebt.add(toBN(dec(1, 18))), alice, alice, { from: alice })
        assert.isFalse(txAlice.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    //repayANTUSD: reduces ANTUSD debt in Trove
    it("repayANTUSD(): reduces the Trove's ANTUSD debt by the correct amount", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      assert.isTrue(aliceDebtBefore.gt(toBN('0')))
    ...
    it("repayANTUSD(): decreases ANTUSD debt in ActivePool by correct amount", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      assert.isTrue(aliceDebtBefore.gt(toBN('0')))
      ...
      // Check before
      const activePool_ANTUSD_Before = await activePool.getANTUSDDebt()
      assert.isTrue(activePool_ANTUSD_Before.gt(toBN('0')))

      await borrowerOperations.repayANTUSD(aliceDebtBefore.div(toBN(10)), alice, alice, { from: alice })  // Repays 1/10 her debt

      // check after
      const activePool_ANTUSD_After = await activePool.getANTUSDDebt()
      th.assertIsApproximatelyEqual(activePool_ANTUSD_After, activePool_ANTUSD_Before.sub(aliceDebtBefore.div(toBN(10))))
    })

    it("repayANTUSD(): decreases user ANTUSDToken balance by correct amount", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      assert.isTrue(aliceDebtBefore.gt(toBN('0')))
    ...
    it('repayANTUSD(): can repay debt in Recovery Mode', async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      assert.isTrue(aliceDebtBefore.gt(toBN('0')))
      ...
      const tx = await borrowerOperations.repayANTUSD(aliceDebtBefore.div(toBN(10)), alice, alice, { from: alice })
    ...
    it("repayANTUSD(): Reverts if borrower has insufficient ANTUSD balance to cover his debt repayment", async () => {
      ...
      // Bob tries to repay 6 ANTUSD
      const repayANTUSDPromise_B = borrowerOperations.repayANTUSD(toBN(dec(6, 18)), B, B, { from: B })

      await assertRevert(repayANTUSDPromise_B, "Caller doesnt have enough ANTUSD to make repayment")
    })
    ...
    it("adjustTrove(): reverts when adjustment would leave trove with ICR < MCR", async () => {
     ...
     await assertRevert(borrowerOperations.adjustTrove(th._100pct, 0, ANTUSDRepayment, false, alice, alice, collTopUp, { from: alice }), 
      "BorrowerOps: An operation that would result in ICR < MCR is not permitted")
    })
    ...
    it("adjustTrove(): doesn't change base rate if it is already zero", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(40, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
      await openTrove({ extraANTUSDAmount: toBN(dec(50, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("adjustTrove(): borrowing at non-zero base records the (drawn debt + fee) on the Trove struct", async () => {
        ...
        const emittedFee = toBN(th.getANTUSDFeeFromANTUSDBorrowingEvent(adjustmentTx))
        assert.isTrue(emittedFee.gt(toBN('0')))
    ...
    it("adjustTrove(): Borrowing at zero base rate sends total requested ANTUSD to the user", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale, value: toBN(dec(100, 'ether')) } })
      await openTrove({ extraANTUSDAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      await openTrove({ extraANTUSDAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })
      await openTrove({ extraANTUSDAmount: toBN(dec(40000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })
    ...
    it("adjustTrove(): reverts when calling address has no active trove", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraANTUSDAmount: toBN(dec(20000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })

      // Alice coll and debt increase(+1 ETH, +50ANTUSD)
    ...
    it("adjustTrove(): debt increase that would reduce the ICR reverts in Recovery Mode", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(3, 18)), extraParams: { from: alice } })
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      const CCR = await troveManager.CCR()
    ...
    it("adjustTrove(): debt increase in Recovery Mode charges no fee", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraANTUSDAmount: toBN(dec(200000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      ...
      // Check emitted fee = 0
      const emittedFee = toBN(await th.getEventArgByName(txAlice, 'ANTUSDBorrowingFeePaid', '_ANTUSDFee'))
    ...
    it("adjustTrove(): reverts when ANTUSD repaid is > debt of the trove", async () => {
      await openTrove({ ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      ...
      const bobFee = toBN(await th.getEventArgByIndex(bobOpenTx, 'ANTUSDBorrowingFeePaid', 1))
    ...
    it("adjustTrove(): reverts when change would cause the ICR of the trove to fall below the MCR", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(100, 18)), extraParams: { from: whale } })

      await priceFeed.setPrice(dec(100, 18))

      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(11, 17)), extraParams: { from: alice } })
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(11, 17)), extraParams: { from: bob } })

      // Bob attempts to increase debt by 100 ANTUSD and 1 ether, i.e. a change that constitutes a 100% ratio of coll:debt.
    ...
    it("adjustTrove(): With 0 coll change, doesnt change borrower's coll or ActivePool coll", async () => {
      ...
      // Alice adjusts trove. No coll change, and a debt increase (+50ANTUSD)
    ...
    it("adjustTrove(): With 0 debt change, doesnt change borrower's debt or ActivePool debt", async () => {
      ...
      const aliceDebtBefore = await getTroveEntireDebt(alice)
      const activePoolDebtBefore = await activePool.getANTUSDDebt()
      ...
      const aliceDebtAfter = await getTroveEntireDebt(alice)
      const activePoolDebtAfter = await activePool.getANTUSDDebt()
    ...
    it("adjustTrove(): updates borrower's debt and coll with an increase in both", async () => {
      ...
      // Alice adjusts trove. Coll and debt increase(+1 ETH, +50ANTUSD)
    ...
    it("adjustTrove(): updates borrower's debt and coll with a decrease in both", async () => {
      ...
      // Alice adjusts trove coll and debt decrease (-0.5 ETH, -50ANTUSD)
    ...
    it("adjustTrove(): updates borrower's  debt and coll with coll increase, debt decrease", async () => {
      ...
      // Alice adjusts trove - coll increase and debt decrease (+0.5 ETH, -50ANTUSD)
    ...
    it("adjustTrove(): updates borrower's debt and coll with coll decrease, debt increase", async () => {
      ...
      // Alice adjusts trove - coll decrease and debt increase (0.1 ETH, 10ANTUSD)
    ...
    it("adjustTrove(): updates borrower's stake and totalStakes with a coll increase", async () => {
      ...
      // Alice adjusts trove - coll and debt increase (+1 ETH, +50 ANTUSD)
      await borrowerOperations.adjustTrove(th._100pct, 0, dec(50, 18), true, alice, alice, dec(1, 'ether'), { from: alice })
    ...
    it("adjustTrove(): changes ANTUSDToken balance by the requested decrease", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    ...
    it("adjustTrove(): changes ANTUSDToken balance by the requested increase", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
    ...
    it("adjustTrove(): Changes the ANTUSD debt in ActivePool by requested decrease", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })

      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })

      const activePool_ANTUSDDebt_Before = await activePool.getANTUSDDebt()
      assert.isTrue(activePool_ANTUSDDebt_Before.gt(toBN('0')))
      ...
      const activePool_ANTUSDDebt_After = await activePool.getANTUSDDebt()
      assert.isTrue(activePool_ANTUSDDebt_After.eq(activePool_ANTUSDDebt_Before.sub(toBN(dec(30, 18)))))
    })

    it("adjustTrove(): Changes the ANTUSD debt in ActivePool by requested increase", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: alice } })
      ...
      th.assertIsApproximatelyEqual(activePool_ANTUSDDebt_After, activePool_ANTUSDDebt_Before.add(toBN(dec(100, 18))))
    })
    ...
    it("adjustTrove(): Reverts if borrower has insufficient ANTUSD balance to cover his debt repayment", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: B } })
      ...
      await assertRevert(repayANTUSDPromise_B, "revert")
    })
    ...
    if (!withProxy) { // no need to test this with proxies
      it("Internal _adjustTrove(): reverts when op is a withdrawal and _borrower param is not the msg.sender", async () => {
        await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
        await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: bob } })
    ...
    it("closeTrove(): reverts when it would lower the TCR below CCR", async () => {
      ...
      await openTrove({ ICR: toBN(dec(120, 16)), extraANTUSDAmount: toBN(dec(300, 18)), extraParams:{ from: bob } })
    ...
    it("closeTrove(): reduces a Trove's collateral to zero", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
    ...
    it("closeTrove(): zero's the troves reward snapshots", async () => {
      ...
      const L_ANTUSDDebt_A_Snapshot = (await troveManager.rewardSnapshots(alice))[1]
      assert.isTrue(L_ETH_A_Snapshot.gt(toBN('0')))
      assert.isTrue(L_ANTUSDDebt_A_Snapshot.gt(toBN('0')))
      ...
      const L_ANTUSDDebt_Snapshot_A_AfterLiquidation = (await troveManager.rewardSnapshots(alice))[1]
      ...
      const L_ANTUSDDebt_Snapshot_A_afterAliceCloses = (await troveManager.rewardSnapshots(alice))[1]

      assert.equal(L_ETH_Snapshot_A_afterAliceCloses, '0')
      assert.equal(L_ANTUSDDebt_Snapshot_A_afterAliceCloses, '0')
    })
    ...
    it("closeTrove(): reduces ActivePool debt by correct amount", async () => {
      ...
      // Check after
      const activePool_Debt_After = (await activePool.getANTUSDDebt()).toString()
    ...
    if (!withProxy) { // TODO: wrap web3.eth.getBalance to be able to go through proxies
      it("closeTrove(): sends the correct amount of ETH to the user", async () => {
        ...
        const alice_ETHBalance_After = web3.utils.toBN(await contracts.stETH.balanceOf(alice))
        const balanceDiff = alice_ETHBalance_After.sub(alice_ETHBalance_Before)
    ...
    it("closeTrove(): subtracts the debt of the closed Trove from the Borrower's ANTUSDToken balance", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: dennis } })
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
    ...
    it("closeTrove(): applies pending rewards", async () => {
      // --- SETUP ---
      await openTrove({ extraANTUSDAmount: toBN(dec(1000000, 18)), ICR: toBN(dec(10, 18)), extraParams: { from: whale } })
      const whaleDebt = await getTroveEntireDebt(whale)
      const whaleColl = await getTroveEntireColl(whale)

      await openTrove({ extraANTUSDAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraANTUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
      ...
      const defaultPool_ANTUSDDebt = await defaultPool.getANTUSDDebt()

      // Carol's liquidated coll (1 ETH) and drawn debt should have entered the Default Pool
      assert.isAtMost(th.getDifference(defaultPool_ETH, liquidatedColl_C), 100)
      assert.isAtMost(th.getDifference(defaultPool_ANTUSDDebt, liquidatedDebt_C), 100)
      ...
      const defaultPool_ANTUSDDebt_afterAliceCloses = await defaultPool.getANTUSDDebt()

      assert.isAtMost(th.getDifference(defaultPool_ETH_afterAliceCloses,
        defaultPool_ETH.sub(pendingCollReward_A)), 1000)
      assert.isAtMost(th.getDifference(defaultPool_ANTUSDDebt_afterAliceCloses,
        defaultPool_ANTUSDDebt.sub(pendingDebtReward_A)), 1000)
      ...
      const defaultPool_ANTUSDDebt_afterBobCloses = await defaultPool.getANTUSDDebt()

      assert.isAtMost(th.getDifference(defaultPool_ETH_afterBobCloses, 0), 100000)
      assert.isAtMost(th.getDifference(defaultPool_ANTUSDDebt_afterBobCloses, 0), 100000)
    })

    it("closeTrove(): reverts if borrower has insufficient ANTUSD balance to repay his entire debt", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })
      ...
      await assertRevert(closeTrovePromise_B, "BorrowerOps: Caller doesnt have enough ANTUSD to make repayment")
    })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("openTrove(): emits a TroveUpdated event with the correct collateral and debt", async () => {
        const txA = (await openTrove({ extraANTUSDAmount: toBN(dec(15000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: A } })).tx
        const txB = (await openTrove({ extraANTUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: B } })).tx
        const txC = (await openTrove({ extraANTUSDAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: C } })).tx
        ...
        const txD = (await openTrove({ extraANTUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })).tx
        const txE = (await openTrove({ extraANTUSDAmount: toBN(dec(3000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })).tx
    ...
    it("openTrove(): decays a non-zero base rate", async () => {
      ...
      // E opens trove 
      await openTrove({ extraANTUSDAmount: toBN(dec(12, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
    ...
    it("openTrove(): lastFeeOpTime doesn't update if less time than decay interval has passed since the last fee operation", async () => {
      ...
      await openTrove({ extraANTUSDAmount: toBN(dec(1, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: D } })
      ...
      await openTrove({ extraANTUSDAmount: toBN(dec(1, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: E } })
    ...
    if (!withProxy) { // TODO: use rawLogs instead of logs
      it("openTrove(): borrowing at non-zero base records the (drawn debt + fee  + liq. reserve) on the Trove struct", async () => {
        ...
        const D_ANTUSDRequest = toBN(dec(20000, 18))

        // D withdraws ANTUSD
        const openTroveTx = await borrowerOperations.openTrove(th._100pct, D_ANTUSDRequest, ZERO_ADDRESS, ZERO_ADDRESS, dec(200, 'ether'), { from: D })

        const emittedFee = toBN(th.getANTUSDFeeFromANTUSDBorrowingEvent(openTroveTx))
        assert.isTrue(toBN(emittedFee).gt(toBN('0')))

        const newDebt = (await troveManager.Troves(D))[0]

        // Check debt on Trove struct equals drawn debt plus emitted fee
        th.assertIsApproximatelyEqual(newDebt, D_ANTUSDRequest.add(emittedFee).add(ANTUSD_GAS_COMPENSATION), 100000)
      })
    }
    ...
    it("openTrove(): Borrowing at zero base rate charges minimum fee", async () => {
      ...
      const ANTUSDRequest = toBN(dec(10000, 18))
      const txC = await borrowerOperations.openTrove(th._100pct, ANTUSDRequest, ZERO_ADDRESS, ZERO_ADDRESS, dec(100, 'ether'), { from: C })
      const _ANTUSDFee = toBN(th.getEventArgByName(txC, "ANTUSDBorrowingFeePaid", "_ANTUSDFee"))

      const expectedFee = BORROWING_FEE_FLOOR.mul(toBN(ANTUSDRequest)).div(toBN(dec(1, 18)))
      assert.isTrue(_ANTUSDFee.eq(expectedFee))
    })

    it("openTrove(): reverts when system is in Recovery Mode and ICR < CCR", async () => {
      await openTrove({ extraANTUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
      await openTrove({ extraANTUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      assert.isFalse(await th.checkRecoveryMode(contracts))
      ...
      try {
        const txBob = await openTrove({ extraANTUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(149, 16)), extraParams: { from: alice } })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("openTrove(): reverts when trove ICR < MCR", async () => {
      ...
      try {
        const txBob = (await openTrove({ extraANTUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(109, 16)), extraParams: { from: bob } })).tx
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
      ...
      try {
        const txBob = await openTrove({ extraANTUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(109, 16)), extraParams: { from: bob } })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("openTrove(): reverts when opening the trove would cause the TCR of the system to fall below the CCR", async () => {
      ...
      await openTrove({ extraANTUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
      ...
      try {
        const txBob = await openTrove({ extraANTUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(149, 16)), extraParams: { from: bob } })
        assert.isFalse(txBob.receipt.status)
      } catch (err) {
        assert.include(err.message, "revert")
      }
    ...
    it("openTrove(): reverts if trove is already active", async () => {
      ...
      await openTrove({ extraANTUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: alice } })
      await openTrove({ extraANTUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: bob } })

      try {
        const txB_1 = await openTrove({ extraANTUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(3, 18)), extraParams: { from: bob } })
    ...
    it("openTrove(): Can open a trove with ICR >= CCR when system is in Recovery Mode", async () => {
      ...
      // price drops to 1ETH:100ANTUSD, reducing TCR below 150%
      await priceFeed.setPrice('100000000000000000000');
      const price = await priceFeed.getPrice()
      ...
      const txCarol = (await openTrove({ extraANTUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(15, 17)), extraParams: { from: carol } })).tx
    ...
    it("openTrove(): creates a new Trove and assigns the correct collateral and debt amount", async () => {
      ...
      const ANTUSDRequest = MIN_NET_DEBT
      borrowerOperations.openTrove(th._100pct, MIN_NET_DEBT, carol, carol, dec(100, 'ether'), { from: alice })

      // Get the expected debt based on the ANTUSD request (adding fee and liq. reserve on top)
      const expectedDebt = ANTUSDRequest
        .add(await troveManager.getBorrowingFee(ANTUSDRequest))
        .add(ANTUSD_GAS_COMPENSATION)
    ...
    it("openTrove(): Increases the activePool ETH and raw ether balance by correct amount", async () => {
      const activePool_ETH_Before = await activePool.getETH()
      const activePool_RawEther_Before = await contracts.stETH.balanceOf(activePool.address)
    ...
    it("openTrove(): records up-to-date initial snapshots of L_ETH and L_ANTUSDDebt", async () => {
      // --- SETUP ---
      ...
      /* with total stakes = 10 ether, after liquidation, L_ETH should equal 1/10 ether per-ether-staked,
       and L_ANTUSD should equal 18 ANTUSD per-ether-staked. */

      const L_ETH = await troveManager.L_ETH()
      const L_ANTUSD = await troveManager.L_ANTUSDDebt()

      assert.isTrue(L_ETH.gt(toBN('0')))
      assert.isTrue(L_ANTUSD.gt(toBN('0')))

      // Bob opens trove
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: bob } })

      // Check Bob's snapshots of L_ETH and L_ANTUSD equal the respective current values
      const bob_rewardSnapshot = await troveManager.rewardSnapshots(bob)
      const bob_ETHrewardSnapshot = bob_rewardSnapshot[0]
      const bob_ANTUSDDebtRewardSnapshot = bob_rewardSnapshot[1]

      assert.isAtMost(th.getDifference(bob_ETHrewardSnapshot, L_ETH), 1000)
      assert.isAtMost(th.getDifference(bob_ANTUSDDebtRewardSnapshot, L_ANTUSD), 1000)
    })

    it("openTrove(): allows a user to open a Trove, then close it, then re-open it", async () => {
      // Open Troves
      await openTrove({ extraANTUSDAmount: toBN(dec(10000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: whale } })
      await openTrove({ extraANTUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: alice } })
      await openTrove({ extraANTUSDAmount: toBN(dec(5000, 18)), ICR: toBN(dec(2, 18)), extraParams: { from: carol } })
    ...
    it("openTrove(): increases the Trove's ANTUSD debt by the correct amount", async () => {
      // check before
      const alice_Trove_Before = await troveManager.Troves(alice)
      const debt_Before = alice_Trove_Before[0]
      assert.equal(debt_Before, 0)

      await borrowerOperations.openTrove(th._100pct, await getOpenTroveANTUSDAmount(dec(10000, 18)), alice, alice, dec(100, 'ether'), { from: alice })
    ...
    it("openTrove(): increases ANTUSD debt in ActivePool by the debt of the trove", async () => {
      const activePool_ANTUSDDebt_Before = await activePool.getANTUSDDebt()
      assert.equal(activePool_ANTUSDDebt_Before, 0)
      ...
      const activePool_ANTUSDDebt_After = await activePool.getANTUSDDebt()
      assert.isTrue(activePool_ANTUSDDebt_After.eq(aliceDebt))
    })
    ...
    it("getCompositeDebt(): returns debt + gas comp", async () => {
      ...
      assert.equal(res1, ANTUSD_GAS_COMPENSATION.toString())

      const res2 = await borrowerOperations.getCompositeDebt(dec(90, 18))
      th.assertIsApproximatelyEqual(res2, ANTUSD_GAS_COMPENSATION.add(toBN(dec(90, 18))))

      const res3 = await borrowerOperations.getCompositeDebt(dec(24423422357345049, 12))
      th.assertIsApproximatelyEqual(res3, ANTUSD_GAS_COMPENSATION.add(toBN(dec(24423422357345049, 12))))
    })
    ...
    describe("getNewTCRFromTroveChange() returns the correct TCR", async () => {
      ...
      it("collChange = 0, debtChange = 0", async () => {
        ...
        const troveANTUSDAmount = await getOpenTroveANTUSDAmount(troveTotalDebt)
        await borrowerOperations.openTrove(th._100pct, troveANTUSDAmount, alice, alice, troveColl, { from: alice })
        await borrowerOperations.openTrove(th._100pct, troveANTUSDAmount, bob, bob, troveColl, { from: bob })
...
/* TODO:
 ...
 changes with addColl, withdrawColl, withdrawANTUSD, repayANTUSD, etc. Can split them up and put them with
