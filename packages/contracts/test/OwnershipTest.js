...
contract('All Liquity functions with onlyOwner modifier', async accounts => {
  ...
  describe('CommunityIssuance', async accounts => {
    it("setAddresses(): reverts when called by non-owner, with wrong addresses, or twice", async () => {
      const params = [antmToken.address, stabilityPool.address]
  ...
  describe('ANTMStaking', async accounts => {
    it("setAddresses(): reverts when called by non-owner, with wrong addresses, or twice", async () => {
      await testSetAddresses(antmStaking, 6)
    })
  })

  describe('LockupContractFactory', async accounts => {
    it("setANTMAddress(): reverts when called by non-owner, with wrong address, or twice", async () => {
      await th.assertRevert(lockupContractFactory.setANTMTokenAddress(antmToken.address, { from: alice }))

      const params = [antmToken.address]

      // Attempt to use zero address
      await testZeroAddress(lockupContractFactory, params, 'setANTMTokenAddress')
      // Attempt to use non contract
      await testNonContractAddress(lockupContractFactory, params, 'setANTMTokenAddress')

      // Owner can successfully set any address
      const txOwner = await lockupContractFactory.setANTMTokenAddress(antmToken.address, { from: owner })

      assert.isTrue(txOwner.receipt.status)
      // fails if called twice
      await th.assertRevert(lockupContractFactory.setANTMTokenAddress(antmToken.address, { from: owner }))
    })
  })
})
