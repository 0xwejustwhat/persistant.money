...
contract('TroveManager - Redistribution reward calculations', async accounts => {
  ...
  it("redistribution: A, B Open. B Liquidated. C, D Open. D Liquidated. Distributes correct rewards", async () => {
    ...
    */
    ...
    // check ANTUSD gas compensation
    assert.equal((await antusdToken.balanceOf(owner)).toString(), dec(400, 18))
  })
  ...
  it("redistribution: Sequence of alternate opening/liquidation: final surviving trove has ETH from all previously liquidated troves", async () => {
    ...
    */
    ...
    // check ANTUSD gas compensation
    assert.equal((await antusdToken.balanceOf(owner)).toString(), dec(1000, 18))
  })
  ...
  it("redistribution: A,B,C,D,E open. Liq(A). B adds coll. Liq(C). B and D have correct coll and debt", async () => {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(100000, 18), extraParams: { from: A } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(100000, 18), extraParams: { from: B } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(100000, 18), extraParams: { from: C } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(20000, 16)), extraANTUSDAmount: dec(10, 18), extraParams: { from: D } })
    const { collateral: E_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(100000, 18), extraParams: { from: E } })
  ...
  it("redistribution: A,B,C Open. Liq(C). B adds coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(110, 18), extraParams: { from: bob } })
    const { collateral: C_coll, totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(110, 18), extraParams: { from: carol } })
    ...
    // Alice withdraws ANTUSD
    await borrowerOperations.withdrawANTUSD(th._100pct, await getNetBorrowingAmount(A_totalDebt), alice, alice, { from: alice })
    ...
    // Expect Bob now holds all Ether and ANTUSDDebt in the system: 2 + 0.4975+0.4975*0.995+0.995 Ether and 110*3 ANTUSD (10 each for gas compensation)
    const bob_Coll = ((await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingETHReward(bob)))
      .toString()

    const bob_ANTUSDDebt = ((await troveManager.Troves(bob))[0]
      .add(await troveManager.getPendingANTUSDDebtReward(bob)))
      .toString()
    ...
    assert.isAtMost(th.getDifference(bob_ANTUSDDebt, A_totalDebt.mul(toBN(2)).add(B_totalDebt).add(C_totalDebt)), 1000)
  })
  ...
  it("redistribution: A,B,C Open. Liq(C). B tops up coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    // D opens trove
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(110, 18), extraParams: { from: dennis } })
    ...
    /* Bob rewards:
     L1: 1/2*0.995 ETH, 55 ANTUSD
     L2: (2.4975/3.995)*0.995 = 0.622 ETH , 110*(2.4975/3.995)= 68.77 ANTUSDDebt

    coll: 3.1195 ETH
    debt: 233.77 ANTUSDDebt

     Alice rewards:
    L1 1/2*0.995 ETH, 55 ANTUSD
    L2 (1.4975/3.995)*0.995 = 0.3730 ETH, 110*(1.4975/3.995) = 41.23 ANTUSDDebt

    coll: 1.8705 ETH
    debt: 146.23 ANTUSDDebt

    totalColl: 4.99 ETH
    totalDebt 380 ANTUSD (includes 50 each for gas compensation)
    */
    const bob_Coll = ((await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingETHReward(bob)))
      .toString()
    ...
    const alice_ANTUSDDebt = ((await troveManager.Troves(alice))[0]
      .add(await troveManager.getPendingANTUSDDebtReward(alice)))
      .toString()
    ...
    assert.isAtMost(th.getDifference(bob_Coll, expected_B_coll), 1000)
    assert.isAtMost(th.getDifference(bob_ANTUSDDebt, expected_B_debt), 10000)
    ...
    assert.isAtMost(th.getDifference(alice_ANTUSDDebt, expected_A_debt), 10000)
  ...
  it("redistribution: Trove with the majority stake tops up. A,B,C, D open. Liq(D). C tops up. E Enters, Liq(E). Distributes correct rewards", async () => {
    ...
    // A, B, C, D open troves
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(400, 16)), extraANTUSDAmount: dec(110, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ extraANTUSDAmount: dec(110, 18), extraParams: { from: carol, value: _998_Ether } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(110, 18), extraParams: { from: dennis, value: dec(1000, 'ether') } })
    ...
    */
    ...
    // check ANTUSD gas compensation
    th.assertIsApproximatelyEqual((await antusdToken.balanceOf(owner)).toString(), dec(400, 18))
  })
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. Liq(A). B acquires all coll and debt", async () => {
    ...
    // Expect Bob now holds all Ether and ANTUSDDebt in the system: 2.5 Ether and 300 ANTUSD
  ...
  it("redistribution: A,B,C Open. Liq(C). B withdraws coll. D Opens. Liq(D). Distributes correct rewards.", async () => {
    ...
    /* Bob rewards:
     L1: 0.4975 ETH, 55 ANTUSD
     L2: (0.9975/2.495)*0.995 = 0.3978 ETH , 110*(0.9975/2.495)= 43.98 ANTUSDDebt

    coll: (1 + 0.4975 - 0.5 + 0.3968) = 1.3953 ETH
    debt: (110 + 55 + 43.98 = 208.98 ANTUSDDebt 

     Alice rewards:
    L1 0.4975, 55 ANTUSD
    L2 (1.4975/2.495)*0.995 = 0.5972 ETH, 110*(1.4975/2.495) = 66.022 ANTUSDDebt

    coll: (1 + 0.4975 + 0.5972) = 2.0947 ETH
    debt: (50 + 55 + 66.022) = 171.022 ANTUSD Debt

    totalColl: 3.49 ETH
    totalDebt 380 ANTUSD (Includes 50 in each trove for gas compensation)
    */
    const bob_Coll = ((await troveManager.Troves(bob))[1]
      .add(await troveManager.getPendingETHReward(bob)))
      .toString()
    ...
    const entireSystemDebt = (await activePool.getANTUSDDebt()).add(await defaultPool.getANTUSDDebt())
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Distributes correct rewards", async () => {
    // A, B, C open troves
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(100, 18), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(100, 18), extraParams: { from: bob } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(100, 18), extraParams: { from: carol } })
    ...
    // E and F open troves
    const { collateral: E_coll, totalDebt: E_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(110, 18), extraParams: { from: erin } })
    const { collateral: F_coll, totalDebt: F_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(110, 18), extraParams: { from: freddy } })
    ...
    // check ANTUSD gas compensation
    assert.equal((await antusdToken.balanceOf(owner)).toString(), dec(600, 18))
  })
  ...
  it("redistribution, all operations: A,B,C open. Liq(A). D opens. B adds, C withdraws. Liq(B). E & F open. D adds. Liq(F). Varying coll. Distributes correct rewards", async () => {
    ...
    */
    ...
    const { collateral: D_coll, totalDebt: D_totalDebt } = await openTrove({ extraANTUSDAmount: dec(100, 18), extraParams: { from: dennis, value: toBN(dec(35, 15)) } })
    ...
    F: 0.0007 ETH
    */
    const { collateral: E_coll, totalDebt: E_totalDebt } = await openTrove({ extraANTUSDAmount: dec(100, 18), extraParams: { from: erin, value: toBN(dec(1, 22)) } })
    const { collateral: F_coll, totalDebt: F_totalDebt } = await openTrove({ extraANTUSDAmount: dec(100, 18), extraParams: { from: freddy, value: toBN('700000000000000') } })
