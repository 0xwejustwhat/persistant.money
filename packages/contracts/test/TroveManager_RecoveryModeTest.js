...
const ANTUSDToken = artifacts.require("./ANTUSDToken.sol")
...
contract('TroveManager - in Recovery Mode', async accounts => {
  ...
  it("checkRecoveryMode(): Returns true if TCR falls below CCR", async () => {
    ...
    // price drops to 1ETH:150ANTUSD, reducing TCR below 150%.  setPrice() calls checkTCRAndSetRecoveryMode() internally.
  ...
  it("liquidate(), with ICR < 100%: removes stake and updates totalStakes", async () => {
    ...
    //  Alice and Bob withdraw such that the TCR is ~150%
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraANTUSDAmount: dec(2000, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with ICR < 100%: updates system snapshots correctly", async () => {
    ...
    // --- TEST ---
    // price drops to 1ETH:100ANTUSD, reducing TCR below 150%, and all Troves below 100% ICR
    await priceFeed.setPrice('100000000000000000000')
  ...
  it("liquidate(), with ICR < 100%: closes the Trove and removes it from the Trove array", async () => {
    ...
    //  Alice and Bob withdraw such that the TCR is ~150%
    await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(150, 16)), extraANTUSDAmount: dec(2000, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with ICR < 100%: only redistributes to active Troves - no offset to Stability Pool", async () => {
    ...
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraANTUSDAmount: spDeposit, extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: dennis } })
  ...
  it("liquidate(), with 100 < ICR < 110%: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    //  Bob withdraws up to 2000 ANTUSD of debt, bringing his ICR to 210%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(210, 16)), extraANTUSDAmount: dec(2000, 18), extraParams: { from: bob } })
    ...
    // --- TEST ---
    // price drops to 1ETH:100ANTUSD, reducing TCR to 117%
    await priceFeed.setPrice('100000000000000000000')
  ...
  it("liquidate(), with 100% < ICR < 110%: updates system snapshots correctly", async () => {
    ...
    //  Bob withdraws up to 20000 ANTUSD of debt, bringing his ICR to 210%
    const { collateral: A_coll, totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: alice } })
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(210, 16)), extraANTUSDAmount: dec(20000, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(150, 16)), extraParams: { from: dennis } })
  ...
  it("liquidate(), with 100% < ICR < 110%: offsets as much debt as possible with the Stability Pool, then redistributes the remainder coll and debt", async () => {
    ...
    // Alice deposits 390ANTUSD to the Stability Pool
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: alice })
    ...
    // check pool ANTUSD before liquidation
    const stabilityPoolANTUSD_Before = (await stabilityPool.getTotalANTUSDDeposits()).toString()
    assert.equal(stabilityPoolANTUSD_Before, '390000000000000000000')
    ...
    /* Now, liquidate Bob. Liquidated coll is 21 ether, and liquidated debt is 2000 ANTUSD.
    
    With 390 ANTUSD in the StabilityPool, 390 ANTUSD should be offset with the pool, leaving 0 in the pool.
  
    Stability Pool rewards for alice should be:
    ANTUSDLoss: 390ANTUSD
    ETHGain: (390 / 2000) * 21*0.995 = 4.074525 ether

    After offsetting 390 ANTUSD and 4.074525 ether, the remainders - 1610 ANTUSD and 16.820475 ether - should be redistributed to all active Troves.
   */
    // Liquidate Bob
    await troveManager.liquidate(bob, { from: owner })

    const aliceDeposit = await stabilityPool.getCompoundedANTUSDDeposit(alice)
    ...
    /* Now, check redistribution to active Troves. Remainders of 1610 ANTUSD and 16.82 ether are distributed.
    ...
    L_ANTUSDDebt = 1610 / 6 = 268.333 ANTUSD
    L_ETH = 16.820475 /6 =  2.8034125 ether
    */
    const L_ANTUSDDebt = (await troveManager.L_ANTUSDDebt()).toString()
    const L_ETH = (await troveManager.L_ETH()).toString()

    assert.isAtMost(th.getDifference(L_ANTUSDDebt, B_totalDebt.sub(spDeposit).mul(mv._1e18BN).div(A_coll.add(D_coll))), 100)
  ...
  it("liquidate(), with ICR > 110%, trove has lowest ICR, and StabilityPool is empty: does nothing", async () => {
    ...
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraANTUSDAmount: dec(2000, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraANTUSDAmount: dec(2000, 18), extraParams: { from: dennis } })
  ...
  // --- liquidate(), applied to trove with ICR > 110% that has the lowest ICR, and Stability Pool ANTUSD is GREATER THAN liquidated debt ---

  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ANTUSD > debt to liquidate: offsets the trove entirely with the pool", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ANTUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ANTUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraANTUSDAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraANTUSDAmount: B_totalDebt, extraParams: { from: alice } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraANTUSDAmount: dec(2000, 18), extraParams: { from: dennis } })

    // Alice deposits ANTUSD in the Stability Pool
    ...
    /* Check accrued Stability Pool rewards after. Total Pool deposits was 1490 ANTUSD, Alice sole depositor.
    As liquidated debt (250 ANTUSD) was completely offset

    Alice's expected compounded deposit: (1490 - 250) = 1240ANTUSD
    Alice's expected ETH gain:  Bob's liquidated capped coll (minus gas comp), 2.75*0.995 ether
  
    */
    const aliceExpectedDeposit = await stabilityPool.getCompoundedANTUSDDeposit(alice)
    ...
    const bob_balanceBefore = th.toBN(await contracts.stETH.balanceOf(bob))
    ...
    const bob_balanceAfter = th.toBN(await contracts.stETH.balanceOf(bob))
    th.assertIsApproximatelyEqual(bob_balanceAfter, bob_expectedBalance.add(th.toBN(bob_remainingCollateral)))
  })

  it("liquidate(), with ICR% = 110 < TCR, and StabilityPool ANTUSD > debt to liquidate: offsets the trove entirely with the pool, there’s no collateral surplus", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ANTUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ANTUSD of debt, resulting in ICR of 220%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(220, 16)), extraANTUSDAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraANTUSDAmount: B_totalDebt, extraParams: { from: alice } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraANTUSDAmount: dec(2000, 18), extraParams: { from: dennis } })
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool ANTUSD > debt to liquidate: removes stake and updates totalStakes", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ANTUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ANTUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraANTUSDAmount: dec(250, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with  110% < ICR < TCR, and StabilityPool ANTUSD > debt to liquidate: updates system snapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ANTUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ANTUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraANTUSDAmount: dec(250, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ANTUSD > debt to liquidate: closes the Trove", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ANTUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ANTUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraANTUSDAmount: dec(250, 18), extraParams: { from: bob } })
  ...
  it("liquidate(), with 110% < ICR < TCR, and StabilityPool ANTUSD > debt to liquidate: can liquidate troves out of order", async () => {
    // taking out 1000 ANTUSD, CR of 200%
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: totalLiquidatedDebt, extraParams: { from: whale } })
    ...
    // can claim collateral
    const dennis_balanceBefore = th.toBN(await contracts.stETH.balanceOf(dennis))
    ...
    const dennis_balanceAfter = th.toBN(await contracts.stETH.balanceOf(dennis))
    ...
    const carol_balanceBefore = th.toBN(await contracts.stETH.balanceOf(carol))
    ...
    const carol_balanceAfter = th.toBN(await contracts.stETH.balanceOf(carol))
  ...
  ANTUSD is LESS THAN the liquidated debt: a non fullfilled liquidation --- */

  it("liquidate(), with ICR > 110%, and StabilityPool ANTUSD < liquidated debt: Trove remains active", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ANTUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ANTUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraANTUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(240, 16)), extraANTUSDAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraANTUSDAmount: dec(2000, 18), extraParams: { from: dennis } })

    // Alice deposits 1490 ANTUSD in the Stability Pool
    ...
    /* Since the pool only contains 100 ANTUSD, and Bob's pre-liquidation debt was 250 ANTUSD,
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ANTUSD < liquidated debt: Trove remains in TroveOwners array", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ANTUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ANTUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraANTUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    // Alice deposits 100 ANTUSD in the Stability Pool
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ANTUSD < liquidated debt: nothing happens", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ANTUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ANTUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraANTUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    /*  Since Bob's debt (250 ANTUSD) is larger than all ANTUSD in the Stability Pool, Liquidation won’t happen
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ANTUSD < liquidated debt: updates system shapshots", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ANTUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ANTUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraANTUSDAmount: dec(1500, 18), extraParams: { from: alice } })
  ...
  it("liquidate(), with ICR > 110%, and StabilityPool ANTUSD < liquidated debt: causes correct Pool offset and ETH gain, and doesn't redistribute to active troves", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ANTUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ANTUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraANTUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    ...
    const L_ANTUSDDebt_After = (await troveManager.L_ANTUSDDebt()).toString()
    const L_ETH_After = (await troveManager.L_ETH()).toString()

    assert.equal(L_ANTUSDDebt_After, '0')
    assert.equal(L_ETH_After, '0')
  })

  it("liquidate(), with ICR > 110%, and StabilityPool ANTUSD < liquidated debt: ICR of non liquidated trove does not change", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ANTUSD of debt, and Dennis up to 150, resulting in ICRs of 266%.
    // Bob withdraws up to 250 ANTUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    // Carol withdraws up to debt of 240 ANTUSD, -> ICR of 250%.
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraANTUSDAmount: dec(1500, 18), extraParams: { from: alice } })
    const { collateral: B_coll } = await openTrove({ ICR: toBN(dec(240, 16)), extraANTUSDAmount: dec(250, 18), extraParams: { from: bob } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraANTUSDAmount: dec(2000, 18), extraParams: { from: dennis } })
    const { collateral: C_coll } = await openTrove({ ICR: toBN(dec(250, 16)), extraANTUSDAmount: dec(240, 18), extraParams: { from: carol } })
    ...
    //Check SP ANTUSD has been completely emptied
    assert.equal((await stabilityPool.getTotalANTUSDDeposits()).toString(), dec(100, 18))
    ...
    await antusdToken.transfer(bob, dec(100, 18), { from: alice })
    ...
    // Alice provides another 50 ANTUSD to pool
    ...
    //Check SP ANTUSD has been completely emptied
    assert.equal((await stabilityPool.getTotalANTUSDDeposits()).toString(), dec(150, 18))
  ...
  it("liquidate() with ICR > 110%, and StabilityPool ANTUSD < liquidated debt: total liquidated coll and debt is correct", async () => {
    // Whale provides 50 ANTUSD to the SP
    await openTrove({ ICR: toBN(dec(300, 16)), extraANTUSDAmount: dec(50, 18), extraParams: { from: whale } })
  ...
  it("liquidate(): Doesn't liquidate undercollateralized trove if it is the only trove in the system", async () => {
    // Alice creates a single trove with 0.62 ETH and a debt of 62 ANTUSD, and provides 10 ANTUSD to SP
  ...
  it("liquidate(): does nothing if trove has >= 110% ICR and the Stability Pool is empty", async () => {
    ...
    // Confirm SP is empty
    const ANTUSDinSP = (await stabilityPool.getTotalANTUSDDeposits()).toString()
    assert.equal(ANTUSDinSP, '0')
  ...
  it("liquidate(): liquidates based on entire/collateral debt (including pending rewards), not raw collateral/debt", async () => {
    ...
    /*  Since there is 0 ANTUSD in the stability Pool, A, with ICR >110%, should stay active.
  ...
  it("liquidate(): does not affect the SP deposit or ETH gain when called on an SP depositor's address that has no trove", async () => {
    ...
    await openTrove({ ICR: toBN(dec(220, 16)), extraANTUSDAmount: spDeposit, extraParams: { from: bob } })

    // Bob sends tokens to Dennis, who has no trove
    await antusdToken.transfer(dennis, spDeposit, { from: bob })

    //Dennis provides 200 ANTUSD to SP
  ...
  it("liquidate(): does not alter the liquidated user's token balance", async () => {
    await openTrove({ ICR: toBN(dec(220, 16)), extraANTUSDAmount: dec(1000, 18), extraParams: { from: whale } })

    const { antusdAmount: A_antusdAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(300, 18), extraParams: { from: alice } })
    const { antusdAmount: B_antusdAmount } = await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(200, 18), extraParams: { from: bob } })
    const { antusdAmount: C_antusdAmount } = await openTrove({ ICR: toBN(dec(206, 16)), extraANTUSDAmount: dec(100, 18), extraParams: { from: carol } })
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, re-open, be reedemed and claim again", async () => {
    // --- SETUP ---
    // Alice withdraws up to 1500 ANTUSD of debt, resulting in ICRs of 266%.
    // Bob withdraws up to 480 ANTUSD of debt, resulting in ICR of 240%. Bob has lowest ICR.
    const { collateral: B_coll, totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(240, 16)), extraANTUSDAmount: dec(480, 18), extraParams: { from: bob } })
    const { collateral: A_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraANTUSDAmount: B_totalDebt, extraParams: { from: alice } })
    ...
    // Bob re-opens the trove, price 200, total debt 80 ANTUSD, ICR = 120% (lowest one)
    // Dennis redeems 30, so Bob has a surplus of (200 * 0.48 - 30) / 200 = 0.33 ETH
    await priceFeed.setPrice('200000000000000000000')
    const { collateral: B_coll_2, netDebt: B_netDebt_2 } = await openTrove({ ICR: toBN(dec(150, 16)), extraANTUSDAmount: dec(480, 18), extraParams: { from: bob, value: bob_remainingCollateral } })
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraANTUSDAmount: B_netDebt_2, extraParams: { from: dennis } })
    ...
    // can claim collateral
    const bob_balanceBefore_2 = th.toBN(await contracts.stETH.balanceOf(bob))
    ...
    const bob_balanceAfter_2 = th.toBN(await contracts.stETH.balanceOf(bob))
  ...
  it("liquidate(), with 110% < ICR < TCR, can claim collateral, after another claim from a redemption", async () => {
    // --- SETUP ---
    // Bob withdraws up to 90 ANTUSD of debt, resulting in ICR of 222%
    const { collateral: B_coll, netDebt: B_netDebt } = await openTrove({ ICR: toBN(dec(222, 16)), extraANTUSDAmount: dec(90, 18), extraParams: { from: bob } })
    // Dennis withdraws to 150 ANTUSD of debt, resulting in ICRs of 266%.
    const { collateral: D_coll } = await openTrove({ ICR: toBN(dec(266, 16)), extraANTUSDAmount: B_netDebt, extraParams: { from: dennis } })
    ...
    // Bob re-opens the trove, price 200, total debt 250 ANTUSD, ICR = 240% (lowest one)
    ...
    // Alice deposits ANTUSD in the Stability Pool
    await openTrove({ ICR: toBN(dec(266, 16)), extraANTUSDAmount: B_totalDebt_2, extraParams: { from: alice } })
  ...
  it("liquidateTroves(): With all ICRs > 110%, Liquidates Troves until system leaves recovery mode", async () => {
    ...
    // Everyone withdraws some ANTUSD from their Trove, resulting in different ICRs
    ...
    const { totalDebt: H_totalDebt } = await openTrove({ ICR: toBN(dec(222, 16)), extraANTUSDAmount: dec(5000, 18), extraParams: { from: harry } })
    const liquidationAmount = E_totalDebt.add(F_totalDebt).add(G_totalDebt).add(H_totalDebt)
    await openTrove({ ICR: toBN(dec(400, 16)), extraANTUSDAmount: liquidationAmount, extraParams: { from: alice } })

    // Alice deposits ANTUSD to Stability Pool
    await stabilityPool.provideToSP(liquidationAmount, ZERO_ADDRESS, { from: alice })

    // price drops
    // price drops to 1ETH:90ANTUSD, reducing TCR below 150%
  ...
  it("liquidateTroves(): Liquidates Troves until 1) system has left recovery mode AND 2) it reaches a Trove with ICR >= 110%", async () => {
    ...
    // price drops to 1ETH:85ANTUSD, reducing TCR below 150%
  ...
  it("liquidateTroves(): does nothing if n = 0", async () => {
    await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(100, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(200, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(300, 18), extraParams: { from: carol } })
  ...
  it("liquidateTroves(): a liquidation sequence containing Pool offsets increases the TCR", async () => {
    ...
    await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(500, 18), extraParams: { from: whale } })
    ...
    await openTrove({ ICR: toBN(dec(198, 16)), extraANTUSDAmount: dec(101, 18), extraParams: { from: defaulter_1 } })
    await openTrove({ ICR: toBN(dec(184, 16)), extraANTUSDAmount: dec(217, 18), extraParams: { from: defaulter_2 } })
    await openTrove({ ICR: toBN(dec(183, 16)), extraANTUSDAmount: dec(328, 18), extraParams: { from: defaulter_3 } })
    await openTrove({ ICR: toBN(dec(186, 16)), extraANTUSDAmount: dec(431, 18), extraParams: { from: defaulter_4 } })
    ...
    // Check Stability Pool has 500 ANTUSD
  ...
  it("liquidateTroves(): A liquidation sequence of pure redistributions decreases the TCR, due to gas compensation, but up to 0.5%", async () => {
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(250, 16)), extraANTUSDAmount: dec(500, 18), extraParams: { from: whale } })
    ...
    const { collateral: d1_coll, totalDebt: d1_totalDebt } = await openTrove({ ICR: toBN(dec(198, 16)), extraANTUSDAmount: dec(101, 18), extraParams: { from: defaulter_1 } })
    const { collateral: d2_coll, totalDebt: d2_totalDebt } = await openTrove({ ICR: toBN(dec(184, 16)), extraANTUSDAmount: dec(217, 18), extraParams: { from: defaulter_2 } })
    const { collateral: d3_coll, totalDebt: d3_totalDebt } = await openTrove({ ICR: toBN(dec(183, 16)), extraANTUSDAmount: dec(328, 18), extraParams: { from: defaulter_3 } })
    const { collateral: d4_coll, totalDebt: d4_totalDebt } = await openTrove({ ICR: toBN(dec(166, 16)), extraANTUSDAmount: dec(431, 18), extraParams: { from: defaulter_4 } })
  ...
  it('liquidateTroves(): does nothing if all troves have ICR > 110% and Stability Pool is empty', async () => {
    ...
    // Confirm 0 ANTUSD in Stability Pool
    assert.equal((await stabilityPool.getTotalANTUSDDeposits()).toString(), '0')
  ...
  it('liquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves', async () => {
    ...
    // Whale adds ANTUSD to SP
    const spDeposit = F_totalDebt.add(G_totalDebt)
    await openTrove({ ICR: toBN(dec(285, 16)), extraANTUSDAmount: spDeposit, extraParams: { from: whale } })
    ...
    // Confirm ANTUSD in Stability Pool
    assert.equal((await stabilityPool.getTotalANTUSDDeposits()).toString(), spDeposit.toString())
    ...
    // can claim collateral
    const freddy_balanceBefore = th.toBN(await contracts.stETH.balanceOf(freddy))
    ...
    const freddy_balanceAfter = th.toBN(await contracts.stETH.balanceOf(freddy))
    th.assertIsApproximatelyEqual(freddy_balanceAfter, freddy_expectedBalance.add(th.toBN(freddy_remainingCollateral)))

    const greta_balanceBefore = th.toBN(await contracts.stETH.balanceOf(greta))
    ...
    const greta_balanceAfter = th.toBN(await contracts.stETH.balanceOf(greta))
  ...
  it('liquidateTroves():  emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    const { collateral: W_coll, totalDebt: W_totalDebt } = await openTrove({ ICR: toBN(dec(285, 16)), extraANTUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    const entireDebt_A = (await troveManager.Troves(alice))[0].add(await troveManager.getPendingANTUSDDebtReward(alice))
  ...
  it("liquidateTroves(): does not affect the liquidated user's token balances", async () => {
    ...
    const { antusdAmount: antusdAmountD } = await openTrove({ ICR: toBN(dec(200, 16)), extraParams: { from: dennis } })
    const { antusdAmount: antusdAmountE } = await openTrove({ ICR: toBN(dec(133, 16)), extraParams: { from: erin } })
    const { antusdAmount: antusdAmountF } = await openTrove({ ICR: toBN(dec(111, 16)), extraParams: { from: freddy } })

    // Check list size is 4
    assert.equal((await sortedTroves.getSize()).toString(), '4')

    // Check token balances before
    assert.equal((await antusdToken.balanceOf(dennis)).toString(), antusdAmountD)
    assert.equal((await antusdToken.balanceOf(erin)).toString(), antusdAmountE)
    assert.equal((await antusdToken.balanceOf(freddy)).toString(), antusdAmountF)
  ...
  it("liquidateTroves(): Liquidating troves at 100 < ICR < 110 with SP deposits correctly impacts their SP deposit and ETH gain", async () => {
    // Whale provides ANTUSD to the SP
    const { antusdAmount: W_antusdAmount } = await openTrove({ ICR: toBN(dec(300, 16)), extraANTUSDAmount: dec(4000, 18), extraParams: { from: whale } })
    await stabilityPool.provideToSP(W_antusdAmount, ZERO_ADDRESS, { from: whale })

    const { antusdAmount: A_antusdAmount, totalDebt: A_totalDebt, collateral: A_coll } = await openTrove({ ICR: toBN(dec(191, 16)), extraANTUSDAmount: dec(40, 18), extraParams: { from: alice } })
    const { antusdAmount: B_antusdAmount, totalDebt: B_totalDebt, collateral: B_coll } = await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: dec(240, 18), extraParams: { from: bob } })
    ...
    await stabilityPool.provideToSP(A_antusdAmount, ZERO_ADDRESS, { from: alice })
    await stabilityPool.provideToSP(B_antusdAmount, ZERO_ADDRESS, { from: bob })

    const totalDeposit = W_antusdAmount.add(A_antusdAmount).add(B_antusdAmount)
    ...
    // Check ANTUSD in Pool
    assert.equal((await stabilityPool.getTotalANTUSDDeposits()).toString(), totalDeposit)
    ...
    Total ANTUSD in Pool: 680 ANTUSD

    Then, liquidation hits A,B,C: 

    Total liquidated debt = 100 + 300 + 100 = 500 ANTUSD
    Total liquidated ETH = 1 + 3 + 1 = 5 ETH

    Whale ANTUSD Loss: 500 * (400/680) = 294.12 ANTUSD
    Alice ANTUSD Loss:  500 *(40/680) = 29.41 ANTUSD
    Bob ANTUSD Loss: 500 * (240/680) = 176.47 ANTUSD

    Whale remaining deposit: (400 - 294.12) = 105.88 ANTUSD
    Alice remaining deposit: (40 - 29.41) = 10.59 ANTUSD
    Bob remaining deposit: (240 - 176.47) = 63.53 ANTUSD

    Whale ETH Gain: 5*0.995 * (400/680) = 2.93 ETH
    Alice ETH Gain: 5*0.995 *(40/680) = 0.293 ETH
    Bob ETH Gain: 5*0.995 * (240/680) = 1.76 ETH

    Total remaining deposits: 180 ANTUSD
    ...
    const whale_Deposit_After = (await stabilityPool.getCompoundedANTUSDDeposit(whale)).toString()
    ...
    assert.isAtMost(th.getDifference(whale_Deposit_After, W_antusdAmount.sub(liquidatedDebt.mul(W_antusdAmount).div(totalDeposit))), 100000)
    assert.isAtMost(th.getDifference(alice_Deposit_After, A_antusdAmount.sub(liquidatedDebt.mul(A_antusdAmount).div(totalDeposit))), 100000)
    assert.isAtMost(th.getDifference(bob_Deposit_After, B_antusdAmount.sub(liquidatedDebt.mul(B_antusdAmount).div(totalDeposit))), 100000)

    assert.isAtMost(th.getDifference(whale_ETHGain, th.applyLiquidationFee(liquidatedColl).mul(W_antusdAmount).div(totalDeposit)), 2000)
    assert.isAtMost(th.getDifference(alice_ETHGain, th.applyLiquidationFee(liquidatedColl).mul(A_antusdAmount).div(totalDeposit)), 2000)
    assert.isAtMost(th.getDifference(bob_ETHGain, th.applyLiquidationFee(liquidatedColl).mul(B_antusdAmount).div(totalDeposit)), 2000)
    ...
    assert.isAtMost(th.getDifference(total_ANTUSDinSP, totalDeposit.sub(liquidatedDebt)), 1000)
  ...
  it("liquidateTroves(): Liquidating troves at ICR <=100% with SP deposits does not alter their deposit or ETH gain", async () => {
    ...
    await openTrove({ ICR: toBN(dec(300, 16)), extraANTUSDAmount: dec(400, 18), extraParams: { from: whale } })
    await stabilityPool.provideToSP(dec(400, 18), ZERO_ADDRESS, { from: whale })

    await openTrove({ ICR: toBN(dec(182, 16)), extraANTUSDAmount: dec(170, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(180, 16)), extraANTUSDAmount: dec(300, 18), extraParams: { from: bob } })
    ...
    // Check ANTUSD and ETH in Pool  before
    const ANTUSDinSP_Before = (await stabilityPool.getTotalANTUSDDeposits()).toString()
    const ETHinSP_Before = (await stabilityPool.getETH()).toString()
    assert.equal(ANTUSDinSP_Before, dec(800, 18))
    ...
    // Check ANTUSD and ETH in Pool after
    const ANTUSDinSP_After = (await stabilityPool.getTotalANTUSDDeposits()).toString()
    const ETHinSP_After = (await stabilityPool.getETH()).toString()
    assert.equal(ANTUSDinSP_Before, ANTUSDinSP_After)
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains active", async () => {
    ...
    await openTrove({ ICR: toBN(dec(300, 16)), extraANTUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    That leaves 50 ANTUSD in the Pool to absorb exactly half of Carol's debt (100) */
    await troveManager.liquidateTroves(10)
  ...
  it("liquidateTroves() with a non fullfilled liquidation: non liquidated trove remains in TroveOwners Array", async () => {
    ...
    // Whale provides ANTUSD to the SP
    const spDeposit = A_totalDebt.add(B_totalDebt).add(C_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(220, 16)), extraANTUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
  ...
  it("liquidateTroves() with a non fullfilled liquidation: still can liquidate further troves after the non-liquidated, emptied pool", async () => {
    ...
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(200, 16)), extraANTUSDAmount: D_totalDebt, extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(208, 16)), extraParams: { from: erin } })
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
     ...
     That leaves 97 ANTUSD in the Pool that won’t be enough to absorb Carol,
  ...
  it("liquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 ANTUSD in the Pool that won’t be enough to absorb any other trove */
    const tx = await troveManager.liquidateTroves(10)

    // Expect system debt reduced by 203 ANTUSD and system coll 2.3 ETH
    const entireSystemCollAfter = await troveManager.getEntireSystemColl()
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt()
  ...
  it("liquidateTroves() with a non fullfilled liquidation: emits correct liquidation event values", async () => {
    ...
    // Whale provides ANTUSD to the SP
    const spDeposit = A_totalDebt.add(B_totalDebt).add(C_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(240, 16)), extraANTUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    /* Liquidate troves. Troves are ordered by ICR, from low to high:  A, B, C, D, E.
    With 253 in the SP, Alice (102 debt) and Bob (101 debt) should be entirely liquidated. 
    That leaves 50 ANTUSD in the Pool which won’t be enough for any other liquidation */
    const liquidationTx = await troveManager.liquidateTroves(10)

    const [liquidatedDebt, liquidatedColl, collGasComp, antusdGasComp] = th.getEmittedLiquidationValues(liquidationTx)
    ...
    assert.equal(antusdGasComp.toString(), dec(400, 18))
    ...
    // can claim collateral
    const alice_balanceBefore = th.toBN(await contracts.stETH.balanceOf(alice))
    const ALICE_GAS = th.gasUsed(await borrowerOperations.claimCollateral({ from: alice, gasPrice: GAS_PRICE  }))
    const alice_balanceAfter = th.toBN(await contracts.stETH.balanceOf(alice))
    th.assertIsApproximatelyEqual(alice_balanceAfter, alice_balanceBefore.add(th.toBN(alice_remainingCollateral).sub(th.toBN(0))))

    const bob_balanceBefore = th.toBN(await contracts.stETH.balanceOf(bob))
    const BOB_GAS = th.gasUsed(await borrowerOperations.claimCollateral({ from: bob, gasPrice: GAS_PRICE  }))
    const bob_balanceAfter = th.toBN(await contracts.stETH.balanceOf(bob))
    th.assertIsApproximatelyEqual(bob_balanceAfter, bob_balanceBefore.add(th.toBN(bob_remainingCollateral).sub(th.toBN(0))))
  })
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    await openTrove({ ICR: toBN(dec(426, 16)), extraANTUSDAmount: spDeposit, extraParams: { from: alice } })
  ...
  it("batchLiquidateTroves(): Liquidates all troves with ICR < 110%, transitioning Normal -> Recovery Mode", async () => {
    ...
    const spDeposit = B_totalDebt.add(C_totalDebt).add(D_totalDebt).add(E_totalDebt).add(F_totalDebt)
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(426, 16)), extraANTUSDAmount: spDeposit, extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(426, 16)), extraANTUSDAmount: A_totalDebt, extraParams: { from: whale } })

    // Alice deposits ANTUSD to Stability Pool
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: alice })

    // to compensate borrowing fee
    await antusdToken.transfer(alice, A_totalDebt, { from: whale })
    // Alice closes trove
  ...
  it("batchLiquidateTroves() with a non fullfilled liquidation: total liquidated coll and debt is correct", async () => {
    ...
    // Expect system debt reduced by 203 ANTUSD and system coll by 2 ETH
    const entireSystemCollAfter = await troveManager.getEntireSystemColl()
    const entireSystemDebtAfter = await troveManager.getEntireSystemDebt()
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool ANTUSD > debt to liquidate: can liquidate troves out of order", async () => {
    ...
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(206, 16)), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(280, 16)), extraANTUSDAmount: dec(500, 18), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(282, 16)), extraANTUSDAmount: dec(500, 18), extraParams: { from: freddy } })

    // Whale provides 1000 ANTUSD to the SP
    const spDeposit = A_totalDebt.add(C_totalDebt).add(D_totalDebt)
    await openTrove({ ICR: toBN(dec(219, 16)), extraANTUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
  ...
  it("batchLiquidateTroves(), with 110% < ICR < TCR, and StabilityPool empty: doesn't liquidate any troves", async () => {
    ...
    const dennisDebt_After = (await troveManager.Troves(dennis))[0].add(await troveManager.getPendingANTUSDDebtReward(dennis))
    const bobDebt_After = (await troveManager.Troves(bob))[0].add(await troveManager.getPendingANTUSDDebtReward(bob))
    const carolDebt_After = (await troveManager.Troves(carol))[0].add(await troveManager.getPendingANTUSDDebtReward(carol))
  ...
  it('batchLiquidateTroves(): skips liquidation of troves with ICR > TCR, regardless of Stability Pool size', async () => {
    ...
    await openTrove({ ICR: toBN(dec(245, 16)), extraANTUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
    ...
    // Whale withdraws entire deposit, and re-deposits 132 ANTUSD
    ...
    // Expected Stability Pool to fully absorb B (92 ANTUSD + 10 virtual debt), 
    // but not E as there are not enough funds in Stability Pool
    
    const stabilityBefore = await stabilityPool.getTotalANTUSDDeposits()
    ...
    const stabilityAfter = await stabilityPool.getTotalANTUSDDeposits()
  ...
  it('batchLiquidateTroves(): emits liquidation event with correct values when all troves have ICR > 110% and Stability Pool covers a subset of troves, including a partial', async () => {
    ...
    // Whale opens trove and adds 220 ANTUSD to SP
    const spDeposit = F_totalDebt.add(G_totalDebt).add(A_totalDebt.div(toBN(2)))
    await openTrove({ ICR: toBN(dec(285, 16)), extraANTUSDAmount: spDeposit, extraParams: { from: whale } })
    await stabilityPool.provideToSP(spDeposit, ZERO_ADDRESS, { from: whale })
