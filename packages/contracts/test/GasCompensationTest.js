...
contract('Gas compensation tests', async accounts => {
  ...
  it('_getCompositeDebt(): returns (debt + 50) when collateral < $10 in value', async () => {
    ...
    debt = 10 ANTUSD
    0.5% of coll = 0.04995 ETH. USD value: $9.99
    -> Expect composite debt = 10 + 200  = 2100 ANTUSD*/
    ...
    /* ETH:USD price = 200
     ...
     debt = 0 ANTUSD
     0.5% of coll = 0.000275 ETH. USD value: $0.055
     -> Expect composite debt = 0 + 200 = 200 ANTUSD*/
    ...
    // debt = 200 ANTUSD 
    // 0.5% of coll = 0.004995 ETH. USD value: $6.09
    // -> Expect  composite debt =  200 + 200 = 400  ANTUSD */
  ...
  it('getCompositeDebt(): returns (debt + 50) collateral = $10 in value', async () => {
    ...
    debt = 123.45 ANTUSD
    0.5% of coll = 0.5 ETH. USD value: $10
    -> Expect composite debt = (123.45 + 200) = 323.45 ANTUSD  */
  ...
  it('getCompositeDebt(): returns (debt + 50) when 0.5% of collateral > $10 in value', async () => {
    ...
    coll = 100 ETH  
    debt = 2000 ANTUSD
    -> Expect composite debt = (2000 + 200) = 2200 ANTUSD  */
    ...
    coll = 37.5 ETH  
    debt = 500 ANTUSD
    -> Expect composite debt = (500 + 200) = 700 ANTUSD  */
    ...
    ETH:USD price = 45323.54542 $/E
    coll = 94758.230582309850 ETH  
    debt = 1 billion ANTUSD
    -> Expect composite debt = (1000000000 + 200) = 1000000200 ANTUSD  */
    await priceFeed.setPrice('45323545420000000000000')
    const price_2 = await priceFeed.getPrice()
    ...
    ETH:USD price = 1000000 $/E (1 million)
    coll = 300000000 ETH   (300 million)
    debt = 54321.123456789 ANTUSD
   -> Expect composite debt = (54321.123456789 + 200) = 54521.123456789 ANTUSD */
    await priceFeed.setPrice(dec(1, 24))
    const price_3 = await priceFeed.getPrice()
  ...
  it('getCurrentICR(): Incorporates virtual debt, and returns the correct ICR for new troves', async () => {
    ...
    // B opens with 0.5 ETH, 50 ANTUSD
    ...
    // F opens with 1 ETH, 100 ANTUSD
    await openTrove({ ICR: toBN(dec(2, 18)), extraANTUSDAmount: dec(100, 18), extraParams: { from: flyn } })
    ...
    // C opens with 2.5 ETH, 160 ANTUSD
    ...
    // E opens with 4405.45 ETH, 32598.35 ANTUSD
  ...
  it('Gas compensation from pool-offset liquidations. All collateral paid as compensation', async () => {
    ...
    // A-E open troves
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraANTUSDAmount: dec(100, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraANTUSDAmount: dec(200, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraANTUSDAmount: dec(300, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraANTUSDAmount: A_totalDebt, extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraANTUSDAmount: B_totalDebt.add(C_totalDebt), extraParams: { from: erin } })

    // D, E each provide ANTUSD to SP
    ...
    const ANTUSDinSP_0 = await stabilityPool.getTotalANTUSDDeposits()
    ...
    const liquidatorBalance_before_A = web3.utils.toBN(await contracts.stETH.balanceOf(liquidator))
    ...
    // Check SP ANTUSD has decreased due to the liquidation 
    const ANTUSDinSP_A = await stabilityPool.getTotalANTUSDDeposits()
    assert.isTrue(ANTUSDinSP_A.lte(ANTUSDinSP_0))
    ...
    const liquidatorBalance_before_B = web3.utils.toBN(await contracts.stETH.balanceOf(liquidator))
    ...
    // Check SP ANTUSD has decreased due to the liquidation of B
    const ANTUSDinSP_B = await stabilityPool.getTotalANTUSDDeposits()
    assert.isTrue(ANTUSDinSP_B.lt(ANTUSDinSP_A))
    ...
    assert.isFalse(await th.checkRecoveryMode(contracts))
    // Liquidate B (use 0 gas price to easily check the amount the compensation amount the liquidator receives)
    const liquidatorBalance_before_C = web3.utils.toBN(await contracts.stETH.balanceOf(liquidator))
    ...
    // Check SP ANTUSD has decreased due to the liquidation of C
    const ANTUSDinSP_C = await stabilityPool.getTotalANTUSDDeposits()
    assert.isTrue(ANTUSDinSP_C.lt(ANTUSDinSP_B))
  ...
  it('gas compensation from pool-offset liquidations: 0.5% collateral < $10 in value. Compensates $10 worth of collateral, liquidates the remainder', async () => {
    ...
    // A-E open troves
    await openTrove({ ICR: toBN(dec(2, 18)), extraANTUSDAmount: dec(200, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(120, 16)), extraANTUSDAmount: dec(5000, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(60, 18)), extraANTUSDAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(80, 18)), extraANTUSDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(80, 18)), extraANTUSDAmount: dec(1, 23), extraParams: { from: erin } })

    // D, E each provide 10000 ANTUSD to SP
    ...
    // Check SP ANTUSD has decreased due to the liquidation of A
    const ANTUSDinSP_A = await stabilityPool.getTotalANTUSDDeposits()
    assert.isTrue(ANTUSDinSP_A.lt(ANTUSDinSP_0))
  ...
  it('gas compensation from pool-offset liquidations: 0.5% collateral > $10 in value. Compensates 0.5% of  collateral, liquidates the remainder', async () => {
    ...
    // A-E open troves
    await openTrove({ ICR: toBN(dec(2, 18)), extraANTUSDAmount: dec(2000, 18), extraParams: { from: alice,} })
    await openTrove({ ICR: toBN(dec(1875, 15)), extraANTUSDAmount: dec(8000, 18), extraParams: { from: bob,} })
    await openTrove({ ICR: toBN(dec(2, 18)), extraANTUSDAmount: dec(600, 18), extraParams: { from: carol} })
    await openTrove({ ICR: toBN(dec(4, 18)), extraANTUSDAmount: dec(1, 23), extraParams: { from: dennis} })
    await openTrove({ ICR: toBN(dec(4, 18)), extraANTUSDAmount: dec(1, 23), extraParams: { from: erin} })
  ...
  it('Gas compensation from pool-offset liquidations. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraANTUSDAmount: dec(100, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(2, 18)), extraANTUSDAmount: dec(200, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraANTUSDAmount: dec(300, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraANTUSDAmount: A_totalDebt, extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(2, 18)), extraANTUSDAmount: B_totalDebt, extraParams: { from: erin } })
  ...
  it('liquidateTroves(): full offset.  Compensates the correct amount, and liquidates the remainder', async () => {
    ...
    // A-F open troves
    await openTrove({ ICR: toBN(dec(118, 16)), extraANTUSDAmount: dec(2000, 18), extraParams: { from: alice } })
    await openTrove({ ICR: toBN(dec(526, 16)), extraANTUSDAmount: dec(8000, 18), extraParams: { from: bob } })
    await openTrove({ ICR: toBN(dec(488, 16)), extraANTUSDAmount: dec(600, 18), extraParams: { from: carol } })
    await openTrove({ ICR: toBN(dec(545, 16)), extraANTUSDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraANTUSDAmount: dec(1, 23), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraANTUSDAmount: dec(1, 23), extraParams: { from: flyn } })
    ...
    */
    ...
    const liquidatorBalance_before = web3.utils.toBN(await contracts.stETH.balanceOf(liquidator))
    const GAS_Used_Liquidator = th.gasUsed(await troveManager.liquidateTroves(4, { from: liquidator, gasPrice: GAS_PRICE }))
    const liquidatorBalance_after = web3.utils.toBN(await contracts.stETH.balanceOf(liquidator))

    // Check ANTUSD in SP has decreased
    const ANTUSDinSP_1 = await stabilityPool.getTotalANTUSDDeposits()
    assert.isTrue(ANTUSDinSP_1.lt(ANTUSDinSP_0))
  ...
  it('liquidateTroves(): full redistribution. Compensates the correct amount, and liquidates the remainder', async () => {
    ...
    const ANTUSDinDefaultPool_0 = await defaultPool.getANTUSDDebt()
    ...
    */
    ...
    // Check ANTUSD in DefaultPool has decreased
    const ANTUSDinDefaultPool_1 = await defaultPool.getANTUSDDebt()
    assert.isTrue(ANTUSDinDefaultPool_1.gt(ANTUSDinDefaultPool_0))
  ...
  it('liquidateTroves(): full offset. Liquidation event emits the correct gas compensation and total liquidated coll and debt', async () => {
    ...
    // A-F open troves
    const { totalDebt: A_totalDebt } = await openTrove({ ICR: toBN(dec(118, 16)), extraANTUSDAmount: dec(2000, 18), extraParams: { from: alice } })
    const { totalDebt: B_totalDebt } = await openTrove({ ICR: toBN(dec(526, 16)), extraANTUSDAmount: dec(8000, 18), extraParams: { from: bob } })
    const { totalDebt: C_totalDebt } = await openTrove({ ICR: toBN(dec(488, 16)), extraANTUSDAmount: dec(600, 18), extraParams: { from: carol } })
    const { totalDebt: D_totalDebt } = await openTrove({ ICR: toBN(dec(545, 16)), extraANTUSDAmount: dec(1, 23), extraParams: { from: dennis } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraANTUSDAmount: dec(1, 23), extraParams: { from: erin } })
    await openTrove({ ICR: toBN(dec(10, 18)), extraANTUSDAmount: dec(1, 23), extraParams: { from: flyn } })
    ...
    */
    ...
    // Expect liquidatedDebt = 51 + 190 + 1025 + 13510 = 14646 ANTUSD
  ...
  it('Trove ordering: same collateral, decreasing debt. Price successively increases. Troves should maintain ordering by ICR', async () => {
    ...
    let debt = 50
    // create 10 troves, constant coll, descending debt 100 to 90 ANTUSD
    for (const account of _10_accounts) {

      const debtString = debt.toString().concat('000000000000000000')
      await openTrove({ extraANTUSDAmount: debtString, extraParams: { from: account, value: dec(30, 'ether') } })
  ...
  it('Trove ordering: increasing collateral, constant debt. Price successively increases. Troves should maintain ordering by ICR', async () => {
    ...
    let coll = 50
    // create 20 troves, increasing collateral, constant debt = 100ANTUSD
    for (const account of _20_accounts) {

      const collString = coll.toString().concat('000000000000000000')
      await openTrove({ extraANTUSDAmount: dec(100, 18), extraParams: { from: account, value: collString } })
