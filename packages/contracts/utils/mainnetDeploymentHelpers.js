...
class MainnetDeploymentHelper {
  ...
  async deployLiquityCoreMainnet(tellorMasterAddr, deploymentState) {
    ...
    const lusdTokenFactory = await this.getFactory("ANTUSDToken")
  ...
  async connectUnipoolMainnet(uniPool, LQTYContracts, ANTUSDWETHPairAddr, duration) {
    const gasPrice = this.configParams.GAS_PRICE
    await this.isOwnershipRenounced(uniPool) ||
      await this.sendAndWaitForTransaction(uniPool.setParams(LQTYContracts.lqtyToken.address, ANTUSDWETHPairAddr, duration, {gasPrice}))
  }
