...
async function main() {
 ...
 const { troveManager, borrowerOperations, hintHelpers, sortedTroves, priceFeedTestnet } = coreContracts
  ...
  const ANTUSDAmount = toBN(toWei('2500')) // borrower wants to withdraw 2500 ANTUSD
  ...
  const liquidationReserve = await troveManager.ANTUSD_GAS_COMPENSATION()
  const expectedFee = await troveManager.getBorrowingFeeWithDecay(ANTUSDAmount)
  
  // Total debt of the new trove = ANTUSD amount drawn, plus fee, plus the liquidation reserve
  const expectedDebt = ANTUSDAmount.add(expectedFee).add(liquidationReserve)
  ...
  await borrowerOperations.openTrove(maxFee, ANTUSDAmount, upperHint, lowerHint, { value: ETHColl })

  // --- adjust trove --- 

  const collIncrease = toBN(toWei('1'))  // borrower wants to add 1 ETH
  const ANTUSDRepayment = toBN(toWei('230')) // borrower wants to repay 230 ANTUSD

  // Get trove's current debt and coll
  const {0: debt, 1: coll} = await troveManager.getEntireDebtAndColl(borrower)
  
  const newDebt = debt.sub(ANTUSDRepayment)
  ...
  await borrowerOperations.adjustTrove(maxFee, 0, ANTUSDRepayment, false, upperHint, lowerHint, {value: collIncrease})


  // --- RedeemCollateral ---

  // Get the redemptions hints from the deployed HintHelpers contract
  const redemptionhint = await hintHelpers.getRedemptionHints(ANTUSDAmount, price, 50)

  const {0: firstRedemptionHint, 1: partialRedemptionNewICR, 2: truncatedANTUSDAmount} = redemptionhint
  ...
  */
  ...
  /* Finally, perform the on-chain redemption, passing the truncated ANTUSD amount, the correct hints, and the expected
  * ICR of the final partially redeemed trove in the sequence. 
  */
  await troveManager.redeemCollateral(truncatedANTUSDAmount,
    firstRedemptionHint,
