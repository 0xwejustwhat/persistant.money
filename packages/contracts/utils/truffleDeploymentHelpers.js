...
const deployLiquity = async () => {
  ...
  const lusdToken = await ANTUSDToken.new(
  ...
  ANTUSDToken.setAsDeployed(lusdToken)
...
const getAddresses = (contracts) => {
  return {
    ...
    ANTUSDToken: contracts.lusdToken.address,
...
const connectContracts = async (contracts, addresses) => {
  ...
  await contracts.troveManager.setANTUSDToken(addresses.ANTUSDToken)
