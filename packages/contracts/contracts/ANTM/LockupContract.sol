...
* Within the first year from deployment, the deployer of the ANTMToken (Liquity AG's address) may transfer ANTM only to valid 
* LockupContracts, and no other addresses (this is enforced in ANTMToken.sol's transfer() function).
* 
* The above two restrictions ensure that until one year after system deployment, ANTM tokens originating from Liquity AG cannot 
...
contract LockupContract {
    ...
    event LockupContractEmptied(uint _ANTMwithdrawal);
    ...
    function withdrawANTM() external {
        _requireCallerIsBeneficiary();
        _requireLockupDurationHasPassed();

        IANTMToken antmTokenCached = antmToken;
        uint ANTMBalance = antmTokenCached.balanceOf(address(this));
        antmTokenCached.transfer(beneficiary, ANTMBalance);
        emit LockupContractEmptied(ANTMBalance);
    }
