...
* 
*
*  --- Functionality added specific to the ANTMToken ---
* 
* 1) Transfer protection: blacklist of addresses that are invalid recipients (i.e. core Liquity contracts) in external 
* transfer() and transferFrom() calls. The purpose is to protect users from losing tokens by mistakenly sending ANTM directly to a Liquity
* core contract, when they should rather call the right function.
*
* 2) sendToANTMStaking(): callable only by Liquity core contracts, which move ANTM tokens from user -> ANTMStaking contract.
*
...
* -sendToANTMStaking() reverts when the multisig is the sender, blocking the multisig from staking its ANTM.
* 
* After one year has passed since deployment of the ANTMToken, the restrictions on multisig operations are lifted
* and the multisig has the same rights as any other address.
*/

contract ANTMToken is CheckContract, IANTMToken {
    using SafeMath for uint256;

    // --- ERC20 Data ---

    string constant internal _NAME = "ANTM";
    string constant internal _SYMBOL = "ANTM";
    ...
    address public immutable antmStakingAddress;
        ...
        public 
    {
        ...
        // Allocate the remainder to the ANTM Multisig: (100 - 2 - 32 - 1.33) million = 64.66 million
    ...
    function sendToANTMStaking(address _sender, uint256 _amount) external override {
        _requireCallerIsANTMStaking();
        if (_isFirstYear()) { _requireSenderIsNotMultisig(_sender); }  // Prevent the multisig from staking ANTM
    ...
    (
        ...
        override 
    {            
        require(deadline >= now, 'ANTM: expired deadline');
        bytes32 digest = keccak256(abi.encodePacked('\x19\x01', 
                         domainSeparator(), keccak256(abi.encode(
                         _PERMIT_TYPEHASH, owner, spender, amount, 
                         _nonces[owner]++, deadline))));
        address recoveredAddress = ecrecover(digest, v, r, s);
        require(recoveredAddress == owner, 'ANTM: invalid signature');
    ...
    function _requireValidRecipient(address _recipient) internal view {
        require(
            ...
            "ANTM: Cannot transfer tokens directly to the ANTM token contract or the zero address"
        );
        require(
            _recipient != communityIssuanceAddress &&
            _recipient != antmStakingAddress,
            "ANTM: Cannot transfer tokens directly to the community issuance or staking contract"
        );
    }
    ...
    function _requireRecipientIsRegisteredLC(address _recipient) internal view {
        ...
        "ANTMToken: recipient must be a LockupContract registered in the Factory");
    }

    function _requireSenderIsNotMultisig(address _sender) internal view {
        require(_sender != multisigAddress, "ANTMToken: sender must not be the multisig");
    }

    function _requireCallerIsNotMultisig() internal view {
        require(!_callerIsMultisig(), "ANTMToken: caller must not be the multisig");
    }

    function _requireCallerIsANTMStaking() internal view {
         require(msg.sender == antmStakingAddress, "ANTMToken: caller must be the ANTMStaking contract");
    }
