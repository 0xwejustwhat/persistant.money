...
/*
 * The Stability Pool holds ANTUSD tokens deposited by Stability Pool depositors.
 *
 * When a trove is liquidated, then depending on system conditions, some of its ANTUSD debt gets offset with
 * ANTUSD in the Stability Pool:  that is, the offset debt evaporates, and an equal amount of ANTUSD tokens in the Stability Pool is burned.
 *
 * Thus, a liquidation causes each depositor to receive a ANTUSD loss, in proportion to their deposit as a share of total deposits.
 * They also receive an ETH gain, as the ETH collateral of the liquidated trove is distributed among Stability depositors,
 * in the same proportion.
 *
 * When a liquidation occurs, it depletes every deposit by the same fraction: for example, a liquidation that depletes 40%
 * of the total ANTUSD in the Stability Pool, depletes 40% of each deposit.
 *
 ...
 * as 0, since it is now less than 1e-9'th of its initial value (e.g. a deposit of 1 billion ANTUSD has depleted to < 1 ANTUSD).
 *
 *
...
contract StabilityPool is LiquityBase, Ownable, CheckContract, IStabilityPool {
    ...
    // Tracker for ANTUSD held in the pool. Changes when users deposit/withdraw, and when Trove debt is offset.
    uint256 internal totalANTUSDDeposits;
    ...
    * after a series of liquidations have occurred, each of which cancel some ANTUSD debt with the deposit.
    *
    ...
    */
    ...
    event StabilityPoolANTUSDBalanceUpdated(uint _newBalance);
    ...
    function getTotalANTUSDDeposits() external view override returns (uint) {
        return totalANTUSDDeposits;
    }
    ...
    */
    function provideToSP(uint _amount, address _frontEndTag) external override {
        ...
        _sendANTUSDtoStabilityPool(msg.sender, _amount);

        uint newDeposit = compoundedANTUSDDeposit.add(_amount);
        _updateDepositAndSnapshots(msg.sender, newDeposit);
        emit UserDepositChanged(msg.sender, newDeposit);

        emit ETHGainWithdrawn(msg.sender, depositorETHGain, ANTUSDLoss); // ANTUSD Loss required for event log
    ...
    */
    function withdrawFromSP(uint _amount) external override {
        ...
        // Update front end stake
        uint compoundedFrontEndStake = getCompoundedFrontEndStake(frontEnd);
        uint newFrontEndStake = compoundedFrontEndStake.sub(ANTUSDtoWithdraw);
        _updateFrontEndStakeAndSnapshots(frontEnd, newFrontEndStake);
        emit FrontEndStakeChanged(frontEnd, newFrontEndStake, msg.sender);

        _sendANTUSDToDepositor(msg.sender, ANTUSDtoWithdraw);

        // Update deposit
        uint newDeposit = compoundedANTUSDDeposit.sub(ANTUSDtoWithdraw);
        _updateDepositAndSnapshots(msg.sender, newDeposit);
        emit UserDepositChanged(msg.sender, newDeposit);
    ...
    * - Updates snapshots for deposit and tagged front end stake */
    function withdrawETHGainToTrove(address _upperHint, address _lowerHint) external override {
        ...
        _updateDepositAndSnapshots(msg.sender, compoundedANTUSDDeposit);

        /* Emit events before transferring ETH gain to Trove.
         This lets the event log make more sense (i.e. so it appears that first the ETH gain is withdrawn
        and then it is deposited into the Trove, not the other way around). */
        emit ETHGainWithdrawn(msg.sender, depositorETHGain, ANTUSDLoss);
        emit UserDepositChanged(msg.sender, compoundedANTUSDDeposit);
    ...
    /*
    * Cancels out the specified debt against the ANTUSD contained in the Stability Pool (as far as possible)
    ...
    */
    function offset(uint _debtToOffset, uint _collToAdd) external override {
        ...
        _updateRewardSumAndProduct(ETHGainPerUnitStaked, ANTUSDLossPerUnitStaked);  // updates S and P
    ...
    function _computeRewardsPerUnitStaked(
        uint _collToAdd,
        uint _debtToOffset,
        uint _totalANTUSDDeposits
    )
        internal
        returns (uint ETHGainPerUnitStaked, uint ANTUSDLossPerUnitStaked)
    {
        /*
        * Compute the ANTUSD and ETH rewards. Uses a "feedback" error correction, to keep
        ...
        */
        ...
        assert(_debtToOffset <= _totalANTUSDDeposits);
        if (_debtToOffset == _totalANTUSDDeposits) {
            ANTUSDLossPerUnitStaked = DECIMAL_PRECISION;  // When the Pool depletes to 0, so does each deposit 
            lastANTUSDLossError_Offset = 0;
        } else {
            uint ANTUSDLossNumerator = _debtToOffset.mul(DECIMAL_PRECISION).sub(lastANTUSDLossError_Offset);
            /*
            * Add 1 to make error in quotient positive. We want "slightly too much" ANTUSD loss,
            * which ensures the error in any given compoundedANTUSDDeposit favors the Stability Pool.
            */
            ANTUSDLossPerUnitStaked = (ANTUSDLossNumerator.div(_totalANTUSDDeposits)).add(1);
            lastANTUSDLossError_Offset = (ANTUSDLossPerUnitStaked.mul(_totalANTUSDDeposits)).sub(ANTUSDLossNumerator);
        }

        ETHGainPerUnitStaked = ETHNumerator.div(_totalANTUSDDeposits);
        lastETHError_Offset = ETHNumerator.sub(ETHGainPerUnitStaked.mul(_totalANTUSDDeposits));

        return (ETHGainPerUnitStaked, ANTUSDLossPerUnitStaked);
    }

    // Update the Stability Pool reward sum S and product P
    function _updateRewardSumAndProduct(uint _ETHGainPerUnitStaked, uint _ANTUSDLossPerUnitStaked) internal {
        uint currentP = P;
        uint newP;

        assert(_ANTUSDLossPerUnitStaked <= DECIMAL_PRECISION);
        /*
        * The newProductFactor is the factor by which to change all deposits, due to the depletion of Stability Pool ANTUSD in the liquidation.
        * We make the product factor 0 if there was a pool-emptying. Otherwise, it is (1 - ANTUSDLossPerUnitStaked)
        */
        uint newProductFactor = uint(DECIMAL_PRECISION).sub(_ANTUSDLossPerUnitStaked);
    ...
    function _moveOffsetCollAndDebt(uint _collToAdd, uint _debtToOffset) internal {
        ...
        // Cancel the liquidated ANTUSD debt with the ANTUSD in the stability pool
        activePoolCached.decreaseANTUSDDebt(_debtToOffset);
        _decreaseANTUSD(_debtToOffset);

        // Burn the debt that was successfully offset
        antusdToken.burn(address(this), _debtToOffset);

        activePoolCached.sendETH(address(this), _collToAdd);
    }

    function _decreaseANTUSD(uint _amount) internal {
        uint newTotalANTUSDDeposits = totalANTUSDDeposits.sub(_amount);
        totalANTUSDDeposits = newTotalANTUSDDeposits;
        emit StabilityPoolANTUSDBalanceUpdated(newTotalANTUSDDeposits);
    }
    ...
    */
    function getCompoundedANTUSDDeposit(address _depositor) public view override returns (uint) {
        uint initialDeposit = deposits[_depositor].initialValue;
        if (initialDeposit == 0) { return 0; }
    ...
    */
    ...
    function _sendANTUSDtoStabilityPool(address _address, uint _amount) internal {
        antusdToken.sendToPool(_address, address(this), _amount);
        uint newTotalANTUSDDeposits = totalANTUSDDeposits.add(_amount);
        totalANTUSDDeposits = newTotalANTUSDDeposits;
        emit StabilityPoolANTUSDBalanceUpdated(newTotalANTUSDDeposits);
    }
    ...
    // Send ANTUSD to user and decrease ANTUSD in Pool
    function _sendANTUSDToDepositor(address _depositor, uint ANTUSDWithdrawal) internal {
        if (ANTUSDWithdrawal == 0) {return;}

        antusdToken.returnFromPool(address(this), _depositor, ANTUSDWithdrawal);
        _decreaseANTUSD(ANTUSDWithdrawal);
    }
